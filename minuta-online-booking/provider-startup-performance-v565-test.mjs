...
const onlineOnlyScripts = ['notification-center.js', 'telegram-auth.js', 'integration-management.js', 'provider-feedback.js'];
assert.ok(assets.length > 0 && assets.length <= 75, `Unexpected core precache size: ${assets.length}`);
for (const asset of assets) {
  ...
  assert.ok(!onlineOnlyScripts.some(script => asset.split('?')[0] === `./${script}`), `${asset} must be runtime-cached`);
...
// The service-actions controller and styles are core UI: keeping them in the
// shell prevents the destructive menu from falling back to its broken layout
// during a cold offline start. Their combined budget is below 20 KB.
assert.ok(precacheBytes <= 3.53 * 1024 * 1024, `Core precache is too large: ${precacheBytes} bytes`);
...
console.log(`Provider startup performance v806: PASS (${assets.length} core files, ${precacheBytes} bytes)`);
