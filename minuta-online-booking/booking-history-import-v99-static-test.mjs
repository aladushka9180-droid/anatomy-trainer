...
assert.match(provider, /function bookingSourceItems\(\)[\s\S]*?const liveRows =[\s\S]*?\[\.\.\.liveRows, \.\.\.importedHistoryForBookingView\(\)\]/, 'Импортированные посещения не добавлены в журнал записей');
