...
assert.match(provider, /theme-catalog\.js\?v=818/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v818`/);
assert.match(worker, /\.\/theme-catalog\.js\?v=818/);
...
assert.ok(neutralScheduleRules.length >= 2, 'Фоновый узор темы не должен попадать в записи и перерывы');
 ...
 console.log('Theme background patterns v818: PASS');
