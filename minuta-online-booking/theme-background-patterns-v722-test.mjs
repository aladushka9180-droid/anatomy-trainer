...
assert.match(provider, /theme-catalog\.js\?v=821/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v821`/);
assert.match(worker, /\.\/theme-catalog\.js\?v=821/);
...
assert.ok(neutralScheduleRules.length >= 2, 'Фоновый узор темы не должен попадать в записи и перерывы');
 ...
 console.log('Theme background patterns v819: PASS');
