...
assert.match(provider, /theme-catalog\.js\?v=817/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v817`/);
assert.match(worker, /\.\/theme-catalog\.js\?v=817/);
...
assert.ok(neutralScheduleRules.length >= 2, 'Фоновый узор темы не должен попадать в записи и перерывы');
 ...
 console.log('Theme background patterns v817: PASS');
