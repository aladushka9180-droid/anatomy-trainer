...
assert.match(provider, /theme-catalog\.js\?v=816/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v816`/);
assert.match(worker, /\.\/theme-catalog\.js\?v=816/);
...
assert.ok(neutralScheduleRules.length >= 2, 'Фоновый узор темы не должен попадать в записи и перерывы');
 ...
 console.log('Theme background patterns v816: PASS');
