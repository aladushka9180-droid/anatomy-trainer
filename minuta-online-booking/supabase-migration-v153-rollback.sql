-- v153 rollback: remove only the exact stamped atomic-create RPC.
begin;
...
begin
  if exists(
    ...
    raise exception using errcode='55000',message='v153_rollback_blocked_partial_or_newer_objects';
  end if;
  ...
  if v_source_hash is distinct from '5920e1426745c8c5425cb15822474952557781b9959d3557f9f3b965e2aae1be'
    ...
    raise exception using errcode='55000',message='v153_rollback_blocked_newer_function_definition';
  end if;
end
$rollback_guard$;

drop function if exists public.book_minuta_appointment_v2(
