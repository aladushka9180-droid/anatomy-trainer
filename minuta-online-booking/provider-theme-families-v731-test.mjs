...
const families = {
  ...
  materials:['luxury', 'loft', 'carbon-ember', 'petrol-steel', 'cobalt-forge', 'concrete-signal', 'obsidian-champagne', 'cocoa-pearl'],
...
assert.match(css, /\[data-provider-layout\]:not\(:is\([^)]*snow-leopard[^)]*pearl-zebra[^)]*concrete-signal[^)]*luxury[^)]*cocoa-pearl[^)]*\)\)::before\s*\{[\s\S]*?pointer-events:none;[\s\S]*?background-image:var\(--theme-canvas-texture\)/);
...
assert.match(css, /\.provider-theme-option\[class\*="theme-"\]:not\(:is\(\.theme-snow-leopard,\.theme-pearl-zebra,\.theme-concrete-signal,\.theme-luxury,\.theme-cocoa-pearl\)\) \.theme-swatch/);
...
assert.match(provider, /provider-theme-families\.css\?v=749[\s\S]*?provider-theme-backgrounds-tema1\.css\?v=749/);
assert.match(worker, /\.\/provider-theme-families\.css\?v=749/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=749/);

console.log('Provider theme families v749: PASS (39 themes, 9 families).');
