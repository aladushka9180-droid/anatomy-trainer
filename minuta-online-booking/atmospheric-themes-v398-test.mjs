...
assert.match(styles, /v398: спокойная атмосферная система для остальных шести тем/, 'Общая атмосферная дизайн-система отсутствует');
...
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v398`;/, 'Кэш приложения не обновлён для атмосферных тем');
assert.match(provider, /styles\.css\?v=398/, 'Кабинет не подключает актуальные стили');

console.log('Atmospheric provider themes v398: OK');
