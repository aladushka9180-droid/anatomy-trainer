...
assert.equal(FakeRecognition.instances.at(-1), noSpeechRecognition, 'no-speech must not restart indefinitely');
assert.equal(noSpeechRecognition.abortCount, 1, 'failed capture must be aborted');
assert.match(status.textContent, /Речь не услышана/);
...
assert.notEqual(earlyEndRecognition, noSpeechRecognition, 'manual retry remains available');
...
await new Promise(resolve => setTimeout(resolve, 260));
assert.equal(FakeRecognition.instances.length, recognitionCountBeforeEarlyEnd, 'empty session must not loop');
assert.match(status.textContent, /не получив речь/);
