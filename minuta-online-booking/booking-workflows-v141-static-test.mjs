...
for (const required of [
  ...
  'bookingRemainingTimeSlots(slots, nearbySlots, selectedTime, step)',
  'bookingRemainingTimeMarkup(newBookingSlots, nearbySlots, newBookingTime)',
  "step === 5 ? 'Ещё варианты' : mobileReference ? `Ещё ${remaining.length} ${mobileCount}` : 'Показать остальные'",
  "Шаг ${step} мин${mobileReference ? '' : ` · ещё ${remaining.length}`}",
  ...
  '<span class="sr-only">Имя клиента</span>${uiIcon(\'search\')}<input id="newBookingName"',
