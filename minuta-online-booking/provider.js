...
function renderProviderAppearanceMenu(colorState = null) {
  ...
  if (icon) icon.setAttribute('href', `ui-icons.svg?v=726#icon-${resolved === 'dark' ? 'moon' : 'sun'}`);
...
function uiIcon(name, className = '') { return `<svg class="ui-icon${className ? ` ${className}` : ''}" aria-hidden="true"><use href="ui-icons.svg?v=726#icon-${name}"></use></svg>`; }
...
async function exportBookingsXlsxInBackground(privacy='masked') {
  ...
  try {
    ...
    worker = new Worker('./report-worker.js?v=726');
...
async function persistTimelineBookingMove(state) {
  ...
  const { error } = await updateBookingAtExpectedState(item, { booking_date:state.date, booking_time:`${targetTime}:00` }, userId);
  timelineMovePending = false;
  if (!sessionIsCurrent(userId, generation)) return;
  if (error) {
