...
function renderProviderAppearanceMenu(colorState = null) {
  ...
  if (icon) icon.setAttribute('href', `ui-icons.svg?v=704#icon-${resolved === 'dark' ? 'moon' : 'sun'}`);
...
function uiIcon(name, className = '') { return `<svg class="ui-icon${className ? ` ${className}` : ''}" aria-hidden="true"><use href="ui-icons.svg?v=704#icon-${name}"></use></svg>`; }
...
async function exportBookingsXlsxInBackground(privacy='masked') {
  ...
  try {
    ...
    worker = new Worker('./report-worker.js?v=704');
...
function synchronizeProvider({ tables = null, background = false } = {}) {
  ...
  const run = (async () => {
    ...
    const offlineSnapshot = primarySnapshotVerified
      ? await saveProviderOfflineSnapshot(userId, generation)
      : await readProviderOfflineSnapshot(userId);
    if (!sessionIsCurrent(userId, generation)) return false;
    offlineBookingAccessReady = Boolean(offlineSnapshot);
    offlineBookingInputsReady = offlineBookingAccessReady || primaryResults.every(result => result?.ok || result?.cached);
    if (!navigator.onLine) {
