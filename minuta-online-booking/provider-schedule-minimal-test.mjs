...
assert.match(styles, /v766: compact schedule controls[\s\S]*grid-template-columns:repeat\(3,minmax\(0,1fr\)\)[\s\S]*min-height:44px!important[\s\S]*calc\(\(100% - 24px\) \/ 5\)[\s\S]*grid-template-columns:44px 44px/s);
assert.match(styles, /v766: narrow schedule polish[\s\S]*grid-template-columns:44px minmax\(0,1fr\) 44px!important[\s\S]*calc\(\(100% - 16px\) \/ 5\)!important[\s\S]*position:static!important/s);
assert.match(styles, /v766: records-list controls keep one quiet row per task[\s\S]*grid-template-rows:auto auto auto!important[\s\S]*grid-row:3!important[\s\S]*border:0!important/s);
...
assert.match(styles, /v766: list controls grow[\s\S]*:has\(\.booking-filters:not\(\[hidden\]\)\)[\s\S]*height:auto!important[\s\S]*\.schedule-controls\s*\{[^}]*display:contents!important;[\s\S]*\.booking-filters\s*\{[^}]*grid-row:2!important;/s);
