...
assert.match(styles, /v783: compact schedule controls[\s\S]*grid-template-columns:repeat\(3,minmax\(0,1fr\)\)[\s\S]*min-height:44px!important[\s\S]*calc\(\(100% - 24px\) \/ 5\)[\s\S]*grid-template-columns:44px 44px/s);
assert.match(styles, /v783: narrow schedule polish[\s\S]*grid-template-columns:44px minmax\(0,1fr\) 44px!important[\s\S]*calc\(\(100% - 16px\) \/ 5\)!important[\s\S]*position:static!important/s);
assert.match(styles, /v783: records-list controls stay compact without nested framing[\s\S]*grid-template-rows:auto auto!important[\s\S]*grid-row:2!important[\s\S]*border:0!important/s);
...
assert.match(styles, /v783: list controls grow[\s\S]*:has\(\.booking-filters:not\(\[hidden\]\)\)[\s\S]*height:auto!important[\s\S]*\.schedule-controls\s*\{[^}]*display:contents!important;[\s\S]*\.booking-filters\s*\{[^}]*grid-row:2!important;/s);
...
assert.match(coreStyles, /v783: compact mobile navigation[\s\S]*bottom:max\(0px,env\(safe-area-inset-bottom,0px\)\)!important;[\s\S]*padding:1px 4px!important;[\s\S]*min-height:44px!important;[\s\S]*gap:1px!important;/s);
