...
assert.match(pearlZebraBackground, /url\("provider-pearl-zebra-smooth-4k-v4\.webp\?v=574"\)/);
...
assert.match(goldenCheetahBackground, /url\("provider-leopard-premium-bg\.webp\?v=574"\)/);
...
assert.match(signatureCss, /@media \(max-width:760px\)[\s\S]*?provider-snow-leopard-mobile-v1\.png\?v=574/);
assert.match(signatureCss, /provider-snow-leopard-mobile-v1\.png\?v=574"\)!important;\s*background-size:100% 100%,100% 100%!important;\s*background-repeat:no-repeat!important;\s*background-position:center top!important;\s*background-attachment:scroll!important/s);
