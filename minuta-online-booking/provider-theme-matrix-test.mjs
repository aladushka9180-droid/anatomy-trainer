...
assert.equal(themes.length * layouts.length, 192, 'the supported appearance matrix must contain 192 combinations');
...
assert.match(pearlZebraBackground, /url\("provider-pearl-zebra-smooth-4k-v4\.webp\?v=691"\)/);
...
assert.match(signatureCss, /@media \(max-width:760px\)[\s\S]*?provider-snow-leopard-mobile-v1\.png\?v=691/);
assert.match(signatureCss, /provider-snow-leopard-mobile-v1\.png\?v=691"\)!important;\s*background-size:100% 100%,100% 100%!important;\s*background-repeat:no-repeat!important;\s*background-position:center top!important;\s*background-attachment:scroll!important/s);
...
console.log('Provider theme matrix checks passed: 32 themes × 6 layouts.');
