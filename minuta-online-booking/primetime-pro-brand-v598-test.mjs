...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=758"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=758"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=758"/);
...
assert.match(worker, /CACHE_PREFIX\}v758/);
assert.match(worker, /provider\.webmanifest\?v=758/);
...
assert.match(provider, /provider\.js\?v=758/);

console.log(`PrimeTime Pro brand v758: PASS (${publicBrandFiles.length} public text surfaces)`);
