...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=741"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=741"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=741"/);
...
assert.match(worker, /CACHE_PREFIX\}v741/);
assert.match(worker, /provider\.webmanifest\?v=741/);
...
assert.match(provider, /provider\.js\?v=741/);

console.log(`PrimeTime Pro brand v741: PASS (${publicBrandFiles.length} public text surfaces)`);
