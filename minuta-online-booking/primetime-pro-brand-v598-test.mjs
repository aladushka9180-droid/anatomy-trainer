...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=784"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=784"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=784"/);
...
assert.match(worker, /CACHE_PREFIX\}v784/);
assert.match(worker, /provider\.webmanifest\?v=784/);
...
assert.match(provider, /provider\.js\?v=784/);

console.log(`PrimeTime Pro brand v784: PASS (${publicBrandFiles.length} public text surfaces)`);
