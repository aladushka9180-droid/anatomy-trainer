...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=779"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=779"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=779"/);
...
assert.match(worker, /CACHE_PREFIX\}v779/);
assert.match(worker, /provider\.webmanifest\?v=779/);
...
assert.match(provider, /provider\.js\?v=779/);

console.log(`PrimeTime Pro brand v779: PASS (${publicBrandFiles.length} public text surfaces)`);
