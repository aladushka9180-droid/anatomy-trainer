...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=774"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=774"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=774"/);
...
assert.match(worker, /CACHE_PREFIX\}v774/);
assert.match(worker, /provider\.webmanifest\?v=774/);
...
assert.match(provider, /provider\.js\?v=774/);

console.log(`PrimeTime Pro brand v774: PASS (${publicBrandFiles.length} public text surfaces)`);
