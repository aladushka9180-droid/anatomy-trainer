...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=748"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=748"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=748"/);
...
assert.match(worker, /CACHE_PREFIX\}v748/);
assert.match(worker, /provider\.webmanifest\?v=748/);
...
assert.match(provider, /provider\.js\?v=748/);

console.log(`PrimeTime Pro brand v748: PASS (${publicBrandFiles.length} public text surfaces)`);
