...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=742"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=742"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=742"/);
...
assert.match(worker, /CACHE_PREFIX\}v742/);
assert.match(worker, /provider\.webmanifest\?v=742/);
...
assert.match(provider, /provider\.js\?v=742/);

console.log(`PrimeTime Pro brand v742: PASS (${publicBrandFiles.length} public text surfaces)`);
