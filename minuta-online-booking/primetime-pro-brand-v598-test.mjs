...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=737"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=737"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=737"/);
...
assert.match(worker, /CACHE_PREFIX\}v737/);
assert.match(worker, /provider\.webmanifest\?v=737/);
...
assert.match(provider, /provider\.js\?v=737/);

console.log(`PrimeTime Pro brand v737: PASS (${publicBrandFiles.length} public text surfaces)`);
