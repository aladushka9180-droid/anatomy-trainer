...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=733"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=733"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=733"/);
...
assert.match(worker, /CACHE_PREFIX\}v733/);
assert.match(worker, /provider\.webmanifest\?v=733/);
...
assert.match(provider, /provider\.js\?v=733/);

console.log(`PrimeTime Pro brand v733: PASS (${publicBrandFiles.length} public text surfaces)`);
