...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=771"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=771"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=771"/);
...
assert.match(worker, /CACHE_PREFIX\}v771/);
assert.match(worker, /provider\.webmanifest\?v=771/);
...
assert.match(provider, /provider\.js\?v=771/);

console.log(`PrimeTime Pro brand v771: PASS (${publicBrandFiles.length} public text surfaces)`);
