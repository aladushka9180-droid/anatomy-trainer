...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=755"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=755"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=755"/);
...
assert.match(worker, /CACHE_PREFIX\}v755/);
assert.match(worker, /provider\.webmanifest\?v=755/);
...
assert.match(provider, /provider\.js\?v=755/);

console.log(`PrimeTime Pro brand v755: PASS (${publicBrandFiles.length} public text surfaces)`);
