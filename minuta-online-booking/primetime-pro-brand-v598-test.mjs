...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=770"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=770"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=770"/);
...
assert.match(worker, /CACHE_PREFIX\}v770/);
assert.match(worker, /provider\.webmanifest\?v=770/);
...
assert.match(provider, /provider\.js\?v=770/);

console.log(`PrimeTime Pro brand v770: PASS (${publicBrandFiles.length} public text surfaces)`);
