...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=783"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=783"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=783"/);
...
assert.match(worker, /CACHE_PREFIX\}v783/);
assert.match(worker, /provider\.webmanifest\?v=783/);
...
assert.match(provider, /provider\.js\?v=783/);

console.log(`PrimeTime Pro brand v783: PASS (${publicBrandFiles.length} public text surfaces)`);
