...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=762"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=762"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=762"/);
...
assert.match(worker, /CACHE_PREFIX\}v762/);
assert.match(worker, /provider\.webmanifest\?v=762/);
...
assert.match(provider, /provider\.js\?v=762/);

console.log(`PrimeTime Pro brand v762: PASS (${publicBrandFiles.length} public text surfaces)`);
