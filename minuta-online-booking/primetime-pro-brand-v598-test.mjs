...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=776"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=776"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=776"/);
...
assert.match(worker, /CACHE_PREFIX\}v776/);
assert.match(worker, /provider\.webmanifest\?v=776/);
...
assert.match(provider, /provider\.js\?v=776/);

console.log(`PrimeTime Pro brand v776: PASS (${publicBrandFiles.length} public text surfaces)`);
