...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=618"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=618"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=618"/);
...
assert.match(worker, /CACHE_PREFIX\}v618/);
assert.match(worker, /provider\.webmanifest\?v=618/);
assert.match(worker, /provider-icon\.svg/);
assert.match(worker, /provider-og\.png/);
assert.match(provider, /provider\.js\?v=618/);

console.log(`PrimeTime Pro brand v618: PASS (${publicBrandFiles.length} public text surfaces)`);
