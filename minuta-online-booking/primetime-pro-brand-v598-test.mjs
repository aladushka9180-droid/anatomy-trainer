...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=683"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=683"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=683"/);
...
assert.match(worker, /CACHE_PREFIX\}v683/);
assert.match(worker, /provider\.webmanifest\?v=683/);
assert.match(worker, /provider-icon\.svg/);
assert.match(worker, /provider-og\.png/);
assert.match(provider, /provider\.js\?v=683/);

console.log(`PrimeTime Pro brand v683: PASS (${publicBrandFiles.length} public text surfaces)`);
