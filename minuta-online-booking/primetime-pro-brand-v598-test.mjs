...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=749"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=749"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=749"/);
...
assert.match(worker, /CACHE_PREFIX\}v749/);
assert.match(worker, /provider\.webmanifest\?v=749/);
...
assert.match(provider, /provider\.js\?v=749/);

console.log(`PrimeTime Pro brand v749: PASS (${publicBrandFiles.length} public text surfaces)`);
