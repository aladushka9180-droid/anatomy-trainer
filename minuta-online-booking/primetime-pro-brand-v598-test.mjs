...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=721"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=721"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=721"/);
...
assert.match(worker, /CACHE_PREFIX\}v721/);
assert.match(worker, /provider\.webmanifest\?v=721/);
...
assert.match(provider, /provider\.js\?v=721/);

console.log(`PrimeTime Pro brand v721: PASS (${publicBrandFiles.length} public text surfaces)`);
