...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=785"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=785"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=785"/);
...
assert.match(worker, /CACHE_PREFIX\}v785/);
assert.match(worker, /provider\.webmanifest\?v=785/);
...
assert.match(provider, /provider\.js\?v=785/);

console.log(`PrimeTime Pro brand v785: PASS (${publicBrandFiles.length} public text surfaces)`);
