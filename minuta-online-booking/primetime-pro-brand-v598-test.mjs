...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=642"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=642"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=642"/);
...
assert.match(worker, /provider\.webmanifest\?v=642/);
assert.match(worker, /provider-icon\.svg/);
assert.match(worker, /provider-og\.png/);
assert.match(provider, /provider\.js\?v=642/);

console.log(`PrimeTime Pro brand v642: PASS (${publicBrandFiles.length} public text surfaces)`);
