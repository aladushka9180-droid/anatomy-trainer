...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=730"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=730"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=730"/);
...
assert.match(worker, /CACHE_PREFIX\}v730/);
assert.match(worker, /provider\.webmanifest\?v=730/);
...
assert.match(provider, /provider\.js\?v=730/);

console.log(`PrimeTime Pro brand v730: PASS (${publicBrandFiles.length} public text surfaces)`);
