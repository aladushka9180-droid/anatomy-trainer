...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=729"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=729"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=729"/);
...
assert.match(worker, /CACHE_PREFIX\}v729/);
assert.match(worker, /provider\.webmanifest\?v=729/);
...
assert.match(provider, /provider\.js\?v=729/);

console.log(`PrimeTime Pro brand v729: PASS (${publicBrandFiles.length} public text surfaces)`);
