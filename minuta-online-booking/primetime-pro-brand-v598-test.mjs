...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=766"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=766"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=766"/);
...
assert.match(worker, /CACHE_PREFIX\}v766/);
assert.match(worker, /provider\.webmanifest\?v=766/);
...
assert.match(provider, /provider\.js\?v=766/);

console.log(`PrimeTime Pro brand v766: PASS (${publicBrandFiles.length} public text surfaces)`);
