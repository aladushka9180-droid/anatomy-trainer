...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=781"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=781"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=781"/);
...
assert.match(worker, /CACHE_PREFIX\}v781/);
assert.match(worker, /provider\.webmanifest\?v=781/);
...
assert.match(provider, /provider\.js\?v=781/);

console.log(`PrimeTime Pro brand v781: PASS (${publicBrandFiles.length} public text surfaces)`);
