...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=625"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=625"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=625"/);
...
assert.match(worker, /CACHE_PREFIX\}v625/);
assert.match(worker, /provider\.webmanifest\?v=625/);
assert.match(worker, /provider-icon\.svg/);
assert.match(worker, /provider-og\.png/);
assert.match(provider, /provider\.js\?v=625/);

console.log(`PrimeTime Pro brand v625: PASS (${publicBrandFiles.length} public text surfaces)`);
