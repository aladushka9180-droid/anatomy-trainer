...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=638"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=638"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=638"/);
...
assert.match(worker, /provider\.webmanifest\?v=638/);
assert.match(worker, /provider-icon\.svg/);
assert.match(worker, /provider-og\.png/);
assert.match(provider, /provider\.js\?v=638/);

console.log(`PrimeTime Pro brand v638: PASS (${publicBrandFiles.length} public text surfaces)`);
