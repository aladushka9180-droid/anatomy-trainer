...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=740"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=740"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=740"/);
...
assert.match(worker, /CACHE_PREFIX\}v740/);
assert.match(worker, /provider\.webmanifest\?v=740/);
...
assert.match(provider, /provider\.js\?v=740/);

console.log(`PrimeTime Pro brand v740: PASS (${publicBrandFiles.length} public text surfaces)`);
