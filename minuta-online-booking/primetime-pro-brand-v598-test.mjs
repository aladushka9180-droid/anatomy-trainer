...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=763"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=763"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=763"/);
...
assert.match(worker, /CACHE_PREFIX\}v763/);
assert.match(worker, /provider\.webmanifest\?v=763/);
...
assert.match(provider, /provider\.js\?v=763/);

console.log(`PrimeTime Pro brand v763: PASS (${publicBrandFiles.length} public text surfaces)`);
