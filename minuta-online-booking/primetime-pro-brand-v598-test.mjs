...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=760"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=760"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=760"/);
...
assert.match(worker, /CACHE_PREFIX\}v760/);
assert.match(worker, /provider\.webmanifest\?v=760/);
...
assert.match(provider, /provider\.js\?v=760/);

console.log(`PrimeTime Pro brand v760: PASS (${publicBrandFiles.length} public text surfaces)`);
