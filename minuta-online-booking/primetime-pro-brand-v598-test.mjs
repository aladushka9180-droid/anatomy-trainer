...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=600"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=600"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=600"/);
...
assert.match(worker, /CACHE_PREFIX\}v600/);
assert.match(worker, /provider\.webmanifest\?v=600/);
assert.match(worker, /provider-icon\.svg/);
assert.match(worker, /provider-og\.png/);
assert.match(provider, /provider\.js\?v=600/);

console.log(`PrimeTime Pro brand v600: PASS (${publicBrandFiles.length} public text surfaces)`);
