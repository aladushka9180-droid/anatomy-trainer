...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=761"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=761"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=761"/);
...
assert.match(worker, /CACHE_PREFIX\}v761/);
assert.match(worker, /provider\.webmanifest\?v=761/);
...
assert.match(provider, /provider\.js\?v=761/);

console.log(`PrimeTime Pro brand v761: PASS (${publicBrandFiles.length} public text surfaces)`);
