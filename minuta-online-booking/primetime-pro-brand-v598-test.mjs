...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=756"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=756"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=756"/);
...
assert.match(worker, /CACHE_PREFIX\}v756/);
assert.match(worker, /provider\.webmanifest\?v=756/);
...
assert.match(provider, /provider\.js\?v=756/);

console.log(`PrimeTime Pro brand v756: PASS (${publicBrandFiles.length} public text surfaces)`);
