...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=695"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=695"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=695"/);
...
assert.match(worker, /CACHE_PREFIX\}v695/);
assert.match(worker, /provider\.webmanifest\?v=695/);
assert.match(worker, /provider-icon\.svg/);
assert.match(worker, /provider-og\.png/);
assert.match(provider, /provider\.js\?v=695/);

console.log(`PrimeTime Pro brand v695: PASS (${publicBrandFiles.length} public text surfaces)`);
