...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=777"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=777"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=777"/);
...
assert.match(worker, /CACHE_PREFIX\}v777/);
assert.match(worker, /provider\.webmanifest\?v=777/);
...
assert.match(provider, /provider\.js\?v=777/);

console.log(`PrimeTime Pro brand v777: PASS (${publicBrandFiles.length} public text surfaces)`);
