...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=751"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=751"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=751"/);
...
assert.match(worker, /CACHE_PREFIX\}v751/);
assert.match(worker, /provider\.webmanifest\?v=751/);
...
assert.match(provider, /provider\.js\?v=751/);

console.log(`PrimeTime Pro brand v751: PASS (${publicBrandFiles.length} public text surfaces)`);
