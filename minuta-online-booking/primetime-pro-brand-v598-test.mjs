...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=692"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=692"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=692"/);
...
assert.match(worker, /CACHE_PREFIX\}v692/);
assert.match(worker, /provider\.webmanifest\?v=692/);
assert.match(worker, /provider-icon\.svg/);
assert.match(worker, /provider-og\.png/);
assert.match(provider, /provider\.js\?v=692/);

console.log(`PrimeTime Pro brand v692: PASS (${publicBrandFiles.length} public text surfaces)`);
