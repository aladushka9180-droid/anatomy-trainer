...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=706"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=706"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=706"/);
...
assert.match(worker, /CACHE_PREFIX\}v706/);
assert.match(worker, /provider\.webmanifest\?v=706/);
assert.match(worker, /provider-icon\.svg/);
assert.match(worker, /provider-og\.png/);
assert.match(provider, /provider\.js\?v=706/);

console.log(`PrimeTime Pro brand v706: PASS (${publicBrandFiles.length} public text surfaces)`);
