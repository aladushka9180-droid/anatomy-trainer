...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=640"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=640"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=640"/);
...
assert.match(worker, /provider\.webmanifest\?v=640/);
assert.match(worker, /provider-icon\.svg/);
assert.match(worker, /provider-og\.png/);
assert.match(provider, /provider\.js\?v=640/);

console.log(`PrimeTime Pro brand v640: PASS (${publicBrandFiles.length} public text surfaces)`);
