...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=767"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=767"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=767"/);
...
assert.match(worker, /CACHE_PREFIX\}v767/);
assert.match(worker, /provider\.webmanifest\?v=767/);
...
assert.match(provider, /provider\.js\?v=767/);

console.log(`PrimeTime Pro brand v767: PASS (${publicBrandFiles.length} public text surfaces)`);
