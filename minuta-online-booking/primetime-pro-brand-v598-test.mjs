...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=621"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=621"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=621"/);
...
assert.match(worker, /CACHE_PREFIX\}v621/);
assert.match(worker, /provider\.webmanifest\?v=621/);
assert.match(worker, /provider-icon\.svg/);
assert.match(worker, /provider-og\.png/);
assert.match(provider, /provider\.js\?v=621/);

console.log(`PrimeTime Pro brand v621: PASS (${publicBrandFiles.length} public text surfaces)`);
