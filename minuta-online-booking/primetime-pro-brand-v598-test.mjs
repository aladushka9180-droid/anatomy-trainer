...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=772"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=772"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=772"/);
...
assert.match(worker, /CACHE_PREFIX\}v772/);
assert.match(worker, /provider\.webmanifest\?v=772/);
...
assert.match(provider, /provider\.js\?v=772/);

console.log(`PrimeTime Pro brand v772: PASS (${publicBrandFiles.length} public text surfaces)`);
