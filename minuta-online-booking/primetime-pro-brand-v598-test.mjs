...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=782"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=782"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=782"/);
...
assert.match(worker, /CACHE_PREFIX\}v782/);
assert.match(worker, /provider\.webmanifest\?v=782/);
...
assert.match(provider, /provider\.js\?v=782/);

console.log(`PrimeTime Pro brand v782: PASS (${publicBrandFiles.length} public text surfaces)`);
