...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=732"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=732"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=732"/);
...
assert.match(worker, /CACHE_PREFIX\}v732/);
assert.match(worker, /provider\.webmanifest\?v=732/);
...
assert.match(provider, /provider\.js\?v=732/);

console.log(`PrimeTime Pro brand v732: PASS (${publicBrandFiles.length} public text surfaces)`);
