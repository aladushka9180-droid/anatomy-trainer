...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=637"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=637"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=637"/);
...
assert.match(worker, /provider\.webmanifest\?v=637/);
assert.match(worker, /provider-icon\.svg/);
assert.match(worker, /provider-og\.png/);
assert.match(provider, /provider\.js\?v=637/);

console.log(`PrimeTime Pro brand v637: PASS (${publicBrandFiles.length} public text surfaces)`);
