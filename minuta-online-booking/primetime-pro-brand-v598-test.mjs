...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=743"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=743"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=743"/);
...
assert.match(worker, /CACHE_PREFIX\}v743/);
assert.match(worker, /provider\.webmanifest\?v=743/);
...
assert.match(provider, /provider\.js\?v=743/);

console.log(`PrimeTime Pro brand v743: PASS (${publicBrandFiles.length} public text surfaces)`);
