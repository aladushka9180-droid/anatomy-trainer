...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=759"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=759"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=759"/);
...
assert.match(worker, /CACHE_PREFIX\}v759/);
assert.match(worker, /provider\.webmanifest\?v=759/);
...
assert.match(provider, /provider\.js\?v=759/);

console.log(`PrimeTime Pro brand v759: PASS (${publicBrandFiles.length} public text surfaces)`);
