...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=757"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=757"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=757"/);
...
assert.match(worker, /CACHE_PREFIX\}v757/);
assert.match(worker, /provider\.webmanifest\?v=757/);
...
assert.match(provider, /provider\.js\?v=757/);

console.log(`PrimeTime Pro brand v757: PASS (${publicBrandFiles.length} public text surfaces)`);
