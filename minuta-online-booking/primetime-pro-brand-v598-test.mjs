...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=752"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=752"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=752"/);
...
assert.match(worker, /CACHE_PREFIX\}v752/);
assert.match(worker, /provider\.webmanifest\?v=752/);
...
assert.match(provider, /provider\.js\?v=752/);

console.log(`PrimeTime Pro brand v752: PASS (${publicBrandFiles.length} public text surfaces)`);
