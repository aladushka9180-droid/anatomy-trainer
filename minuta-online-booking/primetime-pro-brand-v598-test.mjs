...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=773"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=773"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=773"/);
...
assert.match(worker, /CACHE_PREFIX\}v773/);
assert.match(worker, /provider\.webmanifest\?v=773/);
...
assert.match(provider, /provider\.js\?v=773/);

console.log(`PrimeTime Pro brand v773: PASS (${publicBrandFiles.length} public text surfaces)`);
