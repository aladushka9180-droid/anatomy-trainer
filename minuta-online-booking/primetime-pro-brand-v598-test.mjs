...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=775"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=775"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=775"/);
...
assert.match(worker, /CACHE_PREFIX\}v775/);
assert.match(worker, /provider\.webmanifest\?v=775/);
...
assert.match(provider, /provider\.js\?v=775/);

console.log(`PrimeTime Pro brand v775: PASS (${publicBrandFiles.length} public text surfaces)`);
