...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=750"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=750"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=750"/);
...
assert.match(worker, /CACHE_PREFIX\}v750/);
assert.match(worker, /provider\.webmanifest\?v=750/);
...
assert.match(provider, /provider\.js\?v=750/);

console.log(`PrimeTime Pro brand v750: PASS (${publicBrandFiles.length} public text surfaces)`);
