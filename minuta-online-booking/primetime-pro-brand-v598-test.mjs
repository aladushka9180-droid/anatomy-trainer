...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=731"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=731"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=731"/);
...
assert.match(worker, /CACHE_PREFIX\}v731/);
assert.match(worker, /provider\.webmanifest\?v=731/);
...
assert.match(provider, /provider\.js\?v=731/);

console.log(`PrimeTime Pro brand v731: PASS (${publicBrandFiles.length} public text surfaces)`);
