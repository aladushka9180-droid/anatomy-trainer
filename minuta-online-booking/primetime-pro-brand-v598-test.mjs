...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=602"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=602"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=602"/);
...
assert.match(worker, /CACHE_PREFIX\}v602/);
assert.match(worker, /provider\.webmanifest\?v=602/);
assert.match(worker, /provider-icon\.svg/);
assert.match(worker, /provider-og\.png/);
assert.match(provider, /provider\.js\?v=602/);

console.log(`PrimeTime Pro brand v602: PASS (${publicBrandFiles.length} public text surfaces)`);
