...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=734"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=734"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=734"/);
...
assert.match(worker, /CACHE_PREFIX\}v734/);
assert.match(worker, /provider\.webmanifest\?v=734/);
...
assert.match(provider, /provider\.js\?v=734/);

console.log(`PrimeTime Pro brand v734: PASS (${publicBrandFiles.length} public text surfaces)`);
