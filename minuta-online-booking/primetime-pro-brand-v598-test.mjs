...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=765"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=765"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=765"/);
...
assert.match(worker, /CACHE_PREFIX\}v765/);
assert.match(worker, /provider\.webmanifest\?v=765/);
...
assert.match(provider, /provider\.js\?v=765/);

console.log(`PrimeTime Pro brand v765: PASS (${publicBrandFiles.length} public text surfaces)`);
