...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=726"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=726"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=726"/);
...
assert.match(worker, /CACHE_PREFIX\}v726/);
assert.match(worker, /provider\.webmanifest\?v=726/);
...
assert.match(provider, /provider\.js\?v=726/);

console.log(`PrimeTime Pro brand v726: PASS (${publicBrandFiles.length} public text surfaces)`);
