...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=606"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=606"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=606"/);
...
assert.match(worker, /CACHE_PREFIX\}v606/);
assert.match(worker, /provider\.webmanifest\?v=606/);
assert.match(worker, /provider-icon\.svg/);
assert.match(worker, /provider-og\.png/);
assert.match(provider, /provider\.js\?v=606/);

console.log(`PrimeTime Pro brand v606: PASS (${publicBrandFiles.length} public text surfaces)`);
