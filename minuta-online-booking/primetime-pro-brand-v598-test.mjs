...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=720"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=720"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=720"/);
...
assert.match(worker, /CACHE_PREFIX\}v720/);
assert.match(worker, /provider\.webmanifest\?v=720/);
...
assert.match(provider, /provider\.js\?v=720/);

console.log(`PrimeTime Pro brand v720: PASS (${publicBrandFiles.length} public text surfaces)`);
