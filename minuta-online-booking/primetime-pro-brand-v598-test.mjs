...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=778"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=778"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=778"/);
...
assert.match(worker, /CACHE_PREFIX\}v778/);
assert.match(worker, /provider\.webmanifest\?v=778/);
...
assert.match(provider, /provider\.js\?v=778/);

console.log(`PrimeTime Pro brand v778: PASS (${publicBrandFiles.length} public text surfaces)`);
