...
assert.match(provider, /rel="icon" href="provider-icon\.svg\?v=780"/);
assert.match(provider, /property="og:image" content="[^"]+\/provider-og\.png\?v=780"/);
assert.match(provider, /name="twitter:image" content="[^"]+\/provider-og\.png\?v=780"/);
...
assert.match(worker, /CACHE_PREFIX\}v780/);
assert.match(worker, /provider\.webmanifest\?v=780/);
...
assert.match(provider, /provider\.js\?v=780/);

console.log(`PrimeTime Pro brand v780: PASS (${publicBrandFiles.length} public text surfaces)`);
