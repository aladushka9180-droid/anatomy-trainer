-- v152 rollback: remove only the exact stamped recovery RPC; no booking data changes.
begin;
...
begin
  if exists(
    ...
    raise exception using errcode='55000',message='v152_rollback_blocked_partial_or_newer_objects';
  end if;
  ...
  if v_source_hash is distinct from '5d5d7df186a88c9eeaa5a76af88cc5827cdef40419bb621dd05485e8485c7741'
    ...
    raise exception using errcode='55000',message='v152_rollback_blocked_newer_function_definition';
  end if;
end
$rollback_guard$;

drop function if exists public.recover_primetime_booking_request_v1(
