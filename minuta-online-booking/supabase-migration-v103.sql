...
begin
  ...
  with performers as (
    ...
    select performer.performer_id,coalesce((
      select count(distinct schedule.weekday)=7
        and bool_and(not schedule.enabled or (schedule.start_time is not null and schedule.end_time is not null and schedule.end_time>schedule.start_time))
      from public.provider_schedule schedule
      where schedule.performer_id=performer.performer_id and schedule.weekday between 1 and 7
    ),false) has_schedule from performers performer
  ...
  return jsonb_build_object('available_minutes',v_available,'configured_performers',v_configured,'total_performers',v_total,'complete',v_total>0 and v_total=v_configured,'completeness_version',2);
end;
$$;
