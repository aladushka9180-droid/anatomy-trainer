...
const state = { step: 1, services: [], serviceId: '', date: '', time: '', availability: new Map(), loadingAvailability: false };
...
function renderTimes() {
  ...
  if (state.loadingAvailability) {
    $('#times').innerHTML = '<div class="loading-state compact"><i></i><span>Ищем свободное время…</span></div>';
  } else {
...
document.addEventListener('click', event => {
  ...
  if (date && !date.disabled) { state.date = date.dataset.date; state.time = ''; renderDates(); renderTimes(); }
