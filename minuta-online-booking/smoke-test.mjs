...
assert.match(provider, /const step = scheduleStepForDate\(stage\.dataset\.timelineDate \|\| selectedDate\);[\s\S]*?Math\.floor\(rawMinute \/ step\) \* step/, 'Клик по расписанию не учитывает настроенный шаг слотов');
...
assert.equal(timelineTimeFromClick(timelineStage, { clientY: 205 }), '13:25', 'Клик должен выбирать предыдущую доступную отметку шага');
assert.equal(timelineTimeFromClick(timelineStage, { clientY: 239 }), '13:55', 'Конец часа должен учитывать шаг 5 минут');
assert.equal(timelineTimeFromClick(timelineStage, { clientY: 240 }), '14:00', 'Клик ровно в начале часа не должен сдвигаться');
assert.equal(timelineTimeFromClick(timelineStage, { clientY: 515 }), '18:35', 'Клик должен сохранять точный шаг расписания');
assert.equal(timelineTimeFromClick(timelineStage, { clientY: 530 }), '18:50', 'Поздний слот не должен сдвигаться на полчаса назад');
assert.equal(timelineTimeFromClick(timelineStage, { clientY: 389 }), '16:25', 'Клик в 16:29 должен выбирать предыдущую отметку шага 5 минут');
...
const appearanceSources = [
  ...
  `const PROVIDER_DARK_THEME_KEYS = Object.freeze(['graphite','luxury','loft','midnight','botanical','burgundy','mocha-pastel','oled-mono','cobalt-forge','volt-graphite','noir-safari']);`,
...
for (const layout of ['linear', 'soft', 'capsule', 'editorial', 'bento', 'split']) {
  for (const theme of ['luxury', 'loft', 'eco', 'hitech', 'japandi', 'midnight', 'mono', 'desert', 'rose', 'botanical', 'burgundy', 'coastal', 'pearl', 'butter', 'celadon', 'snow-leopard', 'pearl-zebra', 'blue-hydrangea', 'peach-silk', 'moonlit-lilac', 'noir-rose', 'cocoa-pearl', 'plum-cashmere', 'obsidian-champagne', 'carbon-ember', 'petrol-steel', 'carbon-crimson', 'mocha-pastel', 'oled-mono', 'cobalt-forge', 'volt-graphite', 'concrete-signal', 'azure-lagoon', 'noir-safari']) {
