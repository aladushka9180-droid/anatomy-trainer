...
const signatureStyles = readFileSync(join(root, 'provider-themes-signature.css'), 'utf8');
...
assert.match(provider, /bookingReady = primaryResults\.every[\s\S]*setBookingCreationReady\(bookingReady && providerSessionTrust === 'verified'\)[\s\S]*const secondaryResults/, 'Создание записи зависит от необязательных разделов кабинета или неподтверждённой сессии');
...
assert.match(provider, /const timeRangeMarkup = `\$\{startTime\}<span class="timeline-range-separator">–<\/span>\$\{endTime\}`/, 'Карточка записи не рассчитывает интервал начала и окончания');
assert.match(provider, /class="timeline-booking-time"><b>\$\{startTime\}<\/b><small><span class="timeline-range-separator">–<\/span>\$\{endTime\}<\/small>/, 'На большом экране не показывается полный интервал записи');
assert.match(provider, /class="timeline-mobile-time">\$\{timeRangeMarkup\}/, 'На телефоне не показывается полный интервал записи');
assert.match(provider, /const minuteOnly = duration <= 1;/, 'Одноминутная запись не определяется отдельно');
assert.match(provider, /minuteOnly && !mobileTimeline\s*\? `<span class="timeline-booking-copy timeline-booking-minute-copy"><strong><span class="timeline-booking-minute-time">\$\{timeRangeMarkup\}<\/span>[\s\S]*?\$\{serviceTitleMarkup\}<\/strong><\/span>`/, 'Одноминутная запись не показывает время, название и длительность услуги одной строкой');
...
assert.match(provider, /const step = NEW_BOOKING_GRID_MINUTES;[\s\S]*?Math\.floor\(rawMinute \/ step\) \* step/, 'Клик по расписанию не округляется к началу получасового блока');
...
const timelineTimeFromClick = Function(`const NEW_BOOKING_GRID_MINUTES = 30; ${timelineFunctionSource}; return timelineTimeFromClick;`)();
const timelineStage = { dataset: { timelineStart: '600', timelineEnd: '1200' }, getBoundingClientRect: () => ({ top: 100, height: 600 }) };
assert.equal(timelineTimeFromClick(timelineStage, { clientY: 160 }), '11:00', 'Линия 11:00 выбирает начало блока 11:00');
assert.equal(timelineTimeFromClick(timelineStage, { clientY: 189.999 }), '11:00', 'Верхняя половина часа остаётся в блоке 11:00');
assert.equal(timelineTimeFromClick(timelineStage, { clientY: 190 }), '11:30', 'Линия 11:30 выбирает начало блока 11:30');
...
assert.match(provider, /if \(newHour\) \{[\s\S]*?if \(!newBookingTime\.startsWith[\s\S]*?newBookingTime = '';/, 'Выбор часа не должен молча выбирать первое время');
assert.match(provider, /block:\{ durationMinutes:Number\(draft\?\.blockDurationMinutes[\s\S]*?60/, 'Новый перерыв не выбирает 60 минут по умолчанию');
...
assert.match(provider, /\$\{timeRangeMarkup\}\$\{block \? '' : ' · '\}/, 'Перерыв дублирует подпись «Занятое время» после диапазона времени');
...
assert.match(styles, /\.booking-sheet-client-copy>a \{[^}]*margin-top:0;/, 'Телефон снова оторвался от имени клиента');
...
for (const theme of providerThemes) assert.match(allProviderThemeStyles, new RegExp(`data-provider-theme="${theme}"`), `Нет CSS темы ${theme}`);
...
assert.match(providerHtml, /id="reportCommandCenter"[\s\S]*?>Отмечено полученным<[\s\S]*?>Состоялось<[\s\S]*?>Загрузка</, 'Главные показатели статистики снова названы неоднозначно');
assert.match(providerHtml, /data-report-section="money"[\s\S]*?>Стоимость оказанных услуг<[\s\S]*?>Оплата не указана<[\s\S]*?>Подтверждённый долг</, 'Денежная детализация статистики потеряла разделение неизвестной оплаты и подтверждённого долга');
...
assert.match(provider, /data-report-start="\$\{openStart\}" data-report-end="\$\{openEnd\}"[\s\S]*?Открыть записи/, 'Столбцы дохода нельзя открыть в записи выбранного интервала');
assert.match(provider, /reportZeroSummary[\s\S]*?clearMetrics\.map/, 'Нулевые проблемные показатели снова перегружают отчёт');
assert.match(provider, /revenueShare[\s\S]*?% полученного/, 'В услугах не показывается доля подтверждённого получения');
assert.match(provider, /paymentKnownVisits[\s\S]*?Оплата не указана/, 'Неизвестная оплата услуги снова выглядит как нулевая выручка');
...
const appearanceSources = [
  ...
  provider.match(/const AUTOMATIC_BREAK_COLOR_KEYS = [^;]+;/)?.[0],
  ...
  provider.match(/const BOOKING_CARD_DENSITY_KEYS = Object\.freeze\([^;]+;/)?.[0],
  provider.match(/const BOOKING_CARD_DENSITY_PRESETS = Object\.freeze\([\s\S]*?\n\}\);/)?.[0],
...
for (const layout of ['linear', 'soft', 'capsule', 'editorial', 'bento', 'split']) {
  for (const theme of ['luxury', 'loft', 'eco', 'hitech', 'japandi', 'midnight', 'ash-flame', 'mono', 'desert', 'rose', 'botanical', 'burgundy', 'coastal', 'pearl', 'butter', 'celadon', 'snow-leopard', 'pearl-zebra', 'blue-hydrangea', 'peach-silk', 'moonlit-lilac', 'noir-rose', 'cocoa-pearl', 'plum-cashmere', 'obsidian-champagne', 'carbon-ember', 'petrol-steel', 'carbon-crimson', 'mocha-pastel', 'oled-mono', 'cobalt-forge', 'volt-graphite', 'concrete-signal', 'azure-lagoon', 'noir-safari']) {
...
assert.match(provider, /timeline-booking-minute-time[^`]*?\$\{timeRangeMarkup\}[^`]*?\$\{serviceTitleMarkup\}/, 'Время, название и длительность одноминутной записи не собраны в одну строку');
...
assert.match(siteUpdate, /window\.addEventListener\('load', \(\) => checkForUpdate\(\{ force:true \}\)/, 'При открытии страницы не запускается немедленная проверка обновления');
