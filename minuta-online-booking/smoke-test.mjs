...
assert.match(providerHtml, /team-calendar\.js\?v=137/, 'Кабинет не подключает контроллер командного календаря');
assert.match(providerHtml, /resource-management\.js\?v=137/, 'Кабинет не подключает безопасный контроллер ресурсов');
assert.match(providerHtml, /shift-management\.js\?v=137/, 'Кабинет не подключает контроллер смен команды');
...
assert.match(providerHtml, /rel="manifest" href="provider\.webmanifest\?v=137"/, 'Кабинет не подключает собственный устанавливаемый манифест');
...
assert.match(styles, /data-provider-theme="luxury"\] \.provider-brand \.brand-mark \{[^}]*background:linear-gradient\(145deg,#8b591c[^}]*#ffdc82[^}]*color:#211306/, 'Логотип Luxury не использует выразительную золотую заливку');
assert.match(styles, /data-provider-theme="luxury"\] \.user-avatar \{[^}]*background:linear-gradient\(145deg,#8b591c[^}]*#ffdc82[^}]*color:#211306/, 'Аватар мастера Luxury не использует выразительную золотую заливку');
