...
assert.match(providerHtml, /team-calendar\.js\?v=132/, 'Кабинет не подключает контроллер командного календаря');
assert.match(providerHtml, /resource-management\.js\?v=132/, 'Кабинет не подключает безопасный контроллер ресурсов');
...
assert.match(providerHtml, /rel="manifest" href="provider\.webmanifest\?v=132"/, 'Кабинет не подключает собственный устанавливаемый манифест');
