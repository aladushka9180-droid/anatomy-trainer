...
assert.match(providerHtml, /team-calendar\.js\?v=161/, 'Кабинет не подключает контроллер командного календаря');
assert.match(providerHtml, /resource-management\.js\?v=161/, 'Кабинет не подключает безопасный контроллер ресурсов');
assert.match(providerHtml, /shift-management\.js\?v=161/, 'Кабинет не подключает контроллер смен команды');
assert.match(providerHtml, /payroll-management\.js\?v=161/, 'Кабинет не подключает контроллер зарплат');
...
assert.match(providerHtml, /rel="manifest" href="provider\.webmanifest\?v=161"/, 'Кабинет не подключает собственный устанавливаемый манифест');
...
assert.match(styles, /data-provider-theme="luxury"\] :is\(\.provider-empty,\.empty-service\) \{[^}]*background-color:rgba\(7,8,9,\.9\)/, 'Пустые состояния Luxury остаются светлыми');
