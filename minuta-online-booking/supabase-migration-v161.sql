-- v161: disabled-by-default, session-bound PrimeTime message center.
begin;
...
begin
  if to_regclass('public.bookings') is null
    ...
    raise exception using errcode='55000',message='v161_message_center_prerequisites_missing';
  end if;

  select count(*) into v_present from (values
    (to_regclass('public.message_center_settings_v161')),
    (to_regclass('public.message_support_agents_v161')),
    (to_regclass('public.message_conversations_v161')),
    (to_regclass('public.message_participants_v161')),
    (to_regclass('public.conversation_messages_v161')),
    (to_regclass('public.conversation_system_events_v161')),
    (to_regclass('public.conversation_message_actions_v161')),
    (to_regclass('public.message_action_confirmations_v161')),
    (to_regclass('public.message_read_receipts_v161')),
    (to_regclass('public.message_attachments_v161')),
    (to_regclass('public.message_support_requests_v161')),
    (to_regclass('public.message_idempotency_receipts_v161')),
    (to_regclass('public.message_audit_events_v161'))
  ) item(value) where value is not null;
  if v_present not in(0,13) then
    raise exception using errcode='55000',message='v161_message_center_partial_state';
  end if;
  if v_present=13 and exists(
    select 1 from (values
      ('public.message_center_settings_v161'::regclass),
      ('public.message_support_agents_v161'::regclass),
      ('public.message_conversations_v161'::regclass),
      ('public.message_participants_v161'::regclass),
      ('public.conversation_messages_v161'::regclass),
      ('public.conversation_system_events_v161'::regclass),
      ('public.conversation_message_actions_v161'::regclass),
      ('public.message_action_confirmations_v161'::regclass),
      ('public.message_read_receipts_v161'::regclass),
      ('public.message_attachments_v161'::regclass),
      ('public.message_support_requests_v161'::regclass),
      ('public.message_idempotency_receipts_v161'::regclass),
      ('public.message_audit_events_v161'::regclass)
    ) item(value)
    where obj_description(item.value,'pg_class') is distinct from 'minuta_message_center_v161'
  ) then
    raise exception using errcode='55000',message='v161_message_center_newer_state';
  end if;
end
$guard$;

create table if not exists public.message_center_settings_v161(
...
create table if not exists public.message_support_agents_v161(
...
create table if not exists public.message_conversations_v161(
...
create unique index if not exists message_conversations_client_booking_v161_idx
  on public.message_conversations_v161(primary_booking_id)
  where conversation_kind='client';
create index if not exists message_conversations_provider_page_v161_idx
  on public.message_conversations_v161(organization_id,last_activity_at desc,id desc);
create index if not exists message_conversations_client_page_v161_idx
  on public.message_conversations_v161(client_account_id,last_activity_at desc,id desc)
  where client_account_id is not null;

create table if not exists public.message_participants_v161(
  id uuid primary key default gen_random_uuid(),
  conversation_id uuid not null references public.message_conversations_v161(id) on delete cascade,
...
create unique index if not exists message_participants_user_v161_idx
  on public.message_participants_v161(conversation_id,participant_kind,user_id) where user_id is not null;
create unique index if not exists message_participants_account_v161_idx
  on public.message_participants_v161(conversation_id,client_account_id) where client_account_id is not null;
create unique index if not exists message_participants_booking_v161_idx
  on public.message_participants_v161(conversation_id,booking_id) where booking_id is not null;

create table if not exists public.conversation_messages_v161(
  id uuid primary key default gen_random_uuid(),
  conversation_id uuid not null references public.message_conversations_v161(id) on delete cascade,
  sequence bigint not null check(sequence>0),
  sender_participant_id uuid not null references public.message_participants_v161(id) on delete restrict,
...
create index if not exists conversation_messages_page_v161_idx
  on public.conversation_messages_v161(conversation_id,sequence desc);

create table if not exists public.conversation_system_events_v161(
...
create index if not exists conversation_system_events_page_v161_idx
  on public.conversation_system_events_v161(conversation_id,sequence desc);

create table if not exists public.conversation_message_actions_v161(
  id uuid primary key default gen_random_uuid(),
  conversation_id uuid not null references public.message_conversations_v161(id) on delete cascade,
  message_id uuid not null unique references public.conversation_messages_v161(id) on delete cascade,
...
create index if not exists conversation_message_actions_open_v161_idx
  on public.conversation_message_actions_v161(conversation_id,expires_at) where state='proposed';

create table if not exists public.message_action_confirmations_v161(
  id uuid primary key default gen_random_uuid(),
  action_id uuid not null references public.conversation_message_actions_v161(id) on delete restrict,
...
create table if not exists public.message_read_receipts_v161(
  conversation_id uuid not null references public.message_conversations_v161(id) on delete cascade,
  participant_id uuid not null references public.message_participants_v161(id) on delete cascade,
  last_read_sequence bigint not null default 0 check(last_read_sequence>=0),
  updated_at timestamptz not null default now(),
  primary key(conversation_id,participant_id)
);

create table if not exists public.message_attachments_v161(
  id uuid primary key default gen_random_uuid(),
  message_id uuid not null references public.conversation_messages_v161(id) on delete cascade,
...
create table if not exists public.message_support_requests_v161(
  id uuid primary key default gen_random_uuid(),
  conversation_id uuid not null unique references public.message_conversations_v161(id) on delete restrict,
  organization_id uuid references public.organizations(id) on delete restrict,
  requester_participant_id uuid not null references public.message_participants_v161(id) on delete restrict,
...
create index if not exists message_support_requests_queue_v161_idx
  on public.message_support_requests_v161(state,updated_at desc,id desc);

create table if not exists public.message_idempotency_receipts_v161(
...
create table if not exists public.message_audit_events_v161(
  id bigint generated always as identity primary key,
  organization_id uuid references public.organizations(id) on delete restrict,
  conversation_id uuid references public.message_conversations_v161(id) on delete restrict,
  message_id uuid references public.conversation_messages_v161(id) on delete restrict,
  action_id uuid references public.conversation_message_actions_v161(id) on delete restrict,
...
create index if not exists message_audit_scope_v161_idx
  on public.message_audit_events_v161(organization_id,created_at desc,id desc);

do $secure_tables$
declare v_table regclass;
begin
  foreach v_table in array array[
    'public.message_center_settings_v161'::regclass,'public.message_support_agents_v161'::regclass,
    'public.message_conversations_v161'::regclass,'public.message_participants_v161'::regclass,
    'public.conversation_messages_v161'::regclass,'public.conversation_system_events_v161'::regclass,
    'public.conversation_message_actions_v161'::regclass,'public.message_action_confirmations_v161'::regclass,
    'public.message_read_receipts_v161'::regclass,'public.message_attachments_v161'::regclass,
    'public.message_support_requests_v161'::regclass,'public.message_idempotency_receipts_v161'::regclass,
    'public.message_audit_events_v161'::regclass
  ] loop
    ...
    execute format('comment on table %s is %L',v_table,'minuta_message_center_v161');
  end loop;
end
$secure_tables$;

revoke all on sequence public.conversation_system_events_v161_id_seq,
  public.message_audit_events_v161_id_seq from public,anon,authenticated,service_role;

create or replace function public.protect_minuta_message_immutable_v161()
...
create trigger conversation_messages_immutable_v161 before update or delete on public.conversation_messages_v161
for each row execute function public.protect_minuta_message_immutable_v161();
drop trigger if exists conversation_system_events_immutable_v161 on public.conversation_system_events_v161;
create trigger conversation_system_events_immutable_v161 before update or delete on public.conversation_system_events_v161
for each row execute function public.protect_minuta_message_immutable_v161();
drop trigger if exists message_audit_events_immutable_v161 on public.message_audit_events_v161;
create trigger message_audit_events_immutable_v161 before update or delete on public.message_audit_events_v161
for each row execute function public.protect_minuta_message_immutable_v161();

create or replace function public.minuta_message_diagnostics_valid_v161(p_value jsonb)
...
create or replace function public.minuta_message_booking_sha256_v161(p_booking public.bookings)
...
create or replace function public.minuta_message_client_identity_v161(p_session_token text)
...
create or replace function public.minuta_message_provider_role_v161(p_organization uuid)
returns text language sql stable security definer set search_path to '' as $$
...
create or replace function public.minuta_message_provider_can_access_v161(p_conversation uuid)
returns boolean language sql stable security definer set search_path to '' as $$
  select exists(
    select 1 from public.message_conversations_v161 conversation
    ...
    where conversation.id=p_conversation and (
      ...
      or (membership.role='specialist' and (
        booking.performer_id=auth.uid() or exists(select 1 from public.message_participants_v161 participant
...
create or replace function public.minuta_message_client_can_access_v161(p_session_token text,p_conversation uuid)
returns boolean language plpgsql volatile security definer set search_path to '' as $$
declare v_identity record;v_conversation public.message_conversations_v161%rowtype;
begin
  select * into v_identity from public.minuta_message_client_identity_v161(p_session_token);
  if v_identity.session_id is null then return false; end if;
  select * into v_conversation from public.message_conversations_v161 where id=p_conversation;
...
create or replace function public.minuta_message_support_can_access_v161(p_conversation uuid)
returns boolean language sql stable security definer set search_path to '' as $$
  select exists(select 1 from public.message_support_agents_v161 agent
    join public.message_support_requests_v161 request on request.conversation_id=p_conversation
...
create or replace function public.minuta_message_next_sequence_v161(p_conversation uuid)
returns bigint language plpgsql volatile security definer set search_path to '' as $$
declare v_sequence bigint;
begin
  update public.message_conversations_v161 set next_sequence=next_sequence+1,last_activity_at=clock_timestamp()
...
create or replace function public.minuta_message_receipt_v161(
  p_actor_key text,p_operation text,p_request_id uuid,p_request_sha256 text
) returns jsonb language plpgsql stable security definer set search_path to '' as $$
declare v_receipt public.message_idempotency_receipts_v161%rowtype;
begin
  select * into v_receipt from public.message_idempotency_receipts_v161
...
create or replace function public.get_minuta_message_capability_v161(p_organization uuid)
returns jsonb language plpgsql stable security definer set search_path to '' as $$
declare v_role text;v_settings public.message_center_settings_v161%rowtype;
begin
  v_role:=public.minuta_message_provider_role_v161(p_organization);
  if v_role is null then raise exception using errcode='42501',message='message_organization_denied'; end if;
  select * into v_settings from public.message_center_settings_v161 where organization_id=p_organization;
...
create or replace function public.get_minuta_client_message_capability_v161(
...
begin
  ...
  if v_organization is not null then
    ...
    from public.message_center_settings_v161 settings where settings.organization_id=v_organization;
    ...
    with accessible_organizations as (
      ...
      union
      select conversation.organization_id from public.message_conversations_v161 conversation
    ...
    join public.message_center_settings_v161 settings on settings.organization_id=allowed.organization_id;
  end if;
...
create or replace function public.set_minuta_message_center_settings_v161(
...
begin
  ...
  insert into public.message_center_settings_v161(
  ...
  insert into public.message_audit_events_v161(organization_id,actor_kind,actor_ref,event_type,details)
  ...
  return public.get_minuta_message_capability_v161(p_organization);
end
$$;

create or replace function public.set_minuta_support_agent_v161(
...
begin
  ...
  insert into public.message_support_agents_v161(user_id,support_role,active)
  ...
  insert into public.message_audit_events_v161(actor_kind,actor_ref,event_type,details)
...
create or replace function public.minuta_message_ensure_booking_participants_v161(
...
begin
  insert into public.message_participants_v161(
...
create or replace function public.open_minuta_provider_conversation_v161(
...
begin
  ...
  v_role:=public.minuta_message_provider_role_v161(p_organization);
  if v_role is null then raise exception using errcode='42501',message='message_organization_denied'; end if;
  if not exists(select 1 from public.message_center_settings_v161 settings
  ...
  v_response:=public.minuta_message_receipt_v161(v_actor_key,'open_client_conversation',p_request_id,v_hash);
  if v_response is not null then return v_response; end if;
  perform pg_advisory_xact_lock(hashtextextended('message-booking:'||p_booking::text,16002));
  select id into v_conversation from public.message_conversations_v161
  where conversation_kind='client' and primary_booking_id=p_booking;
  if v_conversation is null then
    insert into public.message_conversations_v161(
  ...
  perform public.minuta_message_ensure_booking_participants_v161(v_conversation,v_booking,v_actor,v_role);
  ...
  insert into public.message_idempotency_receipts_v161(actor_key,operation,request_id,request_sha256,response)
  values(v_actor_key,'open_client_conversation',p_request_id,v_hash,v_response);
  insert into public.message_audit_events_v161(organization_id,conversation_id,actor_kind,actor_ref,event_type,request_id,details)
...
create or replace function public.open_minuta_client_conversation_v161(
...
begin
  ...
  perform public.minuta_message_ensure_booking_participants_v161(v_conversation,v_booking,null,null);
...
create or replace function public.minuta_send_message_core_v161(
...
begin
  ...
  if not exists(select 1 from public.message_participants_v161 participant
  ...
  v_response:=public.minuta_message_receipt_v161(p_actor_key,'send_message',p_request_id,v_hash);
  if v_response is not null then return v_response; end if;
  v_sequence:=public.minuta_message_next_sequence_v161(p_conversation);
  insert into public.conversation_messages_v161(
  ...
  select organization_id into v_organization from public.message_conversations_v161 where id=p_conversation;
  ...
  insert into public.message_audit_events_v161(
...
create or replace function public.send_minuta_provider_message_v161(
  p_conversation uuid,p_request_id uuid,p_body text
) returns jsonb language plpgsql volatile security definer set search_path to '' as $$
declare v_actor uuid:=auth.uid();v_conversation public.message_conversations_v161%rowtype;v_role text;v_sender uuid;
begin
  if v_actor is null or not public.minuta_message_provider_can_access_v161(p_conversation) then
    raise exception using errcode='42501',message='message_conversation_denied';
  end if;
  select * into v_conversation from public.message_conversations_v161 where id=p_conversation;
  v_role:=public.minuta_message_provider_role_v161(v_conversation.organization_id);
  if v_conversation.conversation_kind='client' and not exists(select 1 from public.message_center_settings_v161 settings
    where settings.organization_id=v_conversation.organization_id and settings.client_chat_enabled) then
    raise exception using errcode='P0001',message='message_center_disabled';
  end if;
  if v_conversation.conversation_kind='support' and not exists(select 1 from public.message_center_settings_v161 settings
  ...
  insert into public.message_participants_v161(conversation_id,participant_kind,user_id,participant_role)
  values(p_conversation,'organization_user',v_actor,case when v_role in('owner','admin') then v_role else 'specialist' end)
  on conflict do nothing;
  select id into v_sender from public.message_participants_v161
  where conversation_id=p_conversation and participant_kind='organization_user' and user_id=v_actor and active;
  return public.minuta_send_message_core_v161(p_conversation,v_sender,'provider','provider:'||v_actor::text,
    p_request_id,p_body);
end
$$;

create or replace function public.send_minuta_client_message_v161(
  p_session_token text,p_conversation uuid,p_request_id uuid,p_body text
) returns jsonb language plpgsql volatile security definer set search_path to '' as $$
declare v_identity record;v_conversation public.message_conversations_v161%rowtype;v_sender uuid;
begin
  select * into v_identity from public.minuta_message_client_identity_v161(p_session_token);
  if v_identity.session_id is null or not public.minuta_message_client_can_access_v161(p_session_token,p_conversation) then
    raise exception using errcode='42501',message='message_conversation_denied';
  end if;
  select * into v_conversation from public.message_conversations_v161 where id=p_conversation;
  if v_conversation.conversation_kind='client' and not exists(select 1 from public.message_center_settings_v161 settings
  ...
  if v_identity.session_scope='booking' then
    select id into v_sender from public.message_participants_v161 where conversation_id=p_conversation
  ...
  return public.minuta_send_message_core_v161(p_conversation,v_sender,'client',
    'client-session:'||v_identity.session_id::text,p_request_id,p_body);
end
$$;

create or replace function public.minuta_message_timeline_core_v161(
...
begin
  ...
  with entries as (
    ...
    from public.conversation_messages_v161 message
    join public.message_participants_v161 participant on participant.id=message.sender_participant_id
    left join public.conversation_message_actions_v161 action on action.message_id=message.id
    ...
    from public.conversation_system_events_v161 event where event.conversation_id=p_conversation
  ), page as (
    select * from entries
  ...
  if p_before_sequence is not null and v_first is not null then
    select exists(select 1 from (
      select sequence from public.conversation_messages_v161 where conversation_id=p_conversation
      union all select sequence from public.conversation_system_events_v161 where conversation_id=p_conversation
...
create or replace function public.get_minuta_provider_message_timeline_v161(
...
begin
  if auth.uid() is null or not public.minuta_message_provider_can_access_v161(p_conversation) then
    raise exception using errcode='42501',message='message_conversation_denied';
  end if;
  return public.minuta_message_timeline_core_v161(p_conversation,p_after_sequence,p_before_sequence,p_limit);
end
$$;

create or replace function public.get_minuta_client_message_timeline_v161(
...
begin
  if not public.minuta_message_client_can_access_v161(p_session_token,p_conversation) then
    raise exception using errcode='42501',message='message_conversation_denied';
  end if;
  return public.minuta_message_timeline_core_v161(p_conversation,p_after_sequence,p_before_sequence,p_limit);
end
...
create or replace function public.minuta_mark_message_read_core_v161(
  p_conversation uuid,p_participant uuid,p_sequence bigint
) returns jsonb language plpgsql volatile security definer set search_path to '' as $$
declare v_max bigint;v_value bigint;
begin
  select next_sequence into v_max from public.message_conversations_v161 where id=p_conversation;
  ...
  insert into public.message_read_receipts_v161(conversation_id,participant_id,last_read_sequence)
  values(p_conversation,p_participant,v_value)
  on conflict(conversation_id,participant_id) do update
    set last_read_sequence=greatest(public.message_read_receipts_v161.last_read_sequence,excluded.last_read_sequence),updated_at=now()
...
create or replace function public.mark_minuta_provider_message_read_v161(p_conversation uuid,p_sequence bigint)
returns jsonb language plpgsql volatile security definer set search_path to '' as $$
declare v_actor uuid:=auth.uid();v_participant uuid;v_conversation public.message_conversations_v161%rowtype;v_role text;
begin
  if v_actor is null or not public.minuta_message_provider_can_access_v161(p_conversation) then
    raise exception using errcode='42501',message='message_conversation_denied';
  end if;
  ...
  values(p_conversation,'organization_user',v_actor,case when v_role in('owner','admin') then v_role else 'specialist' end)
  on conflict do nothing;
  select id into v_participant from public.message_participants_v161 where conversation_id=p_conversation and user_id=v_actor and active;
  return public.minuta_mark_message_read_core_v161(p_conversation,v_participant,p_sequence);
end
$$;

create or replace function public.mark_minuta_client_message_read_v161(
...
begin
  ...
  if v_identity.session_scope='booking' then
    select id into v_participant from public.message_participants_v161 where conversation_id=p_conversation
      and participant_kind='booking_client' and booking_id=v_identity.claimed_booking_id and active;
  else
    select id into v_participant from public.message_participants_v161 where conversation_id=p_conversation
      and participant_kind='client_account' and client_account_id=v_identity.client_account_id and active;
...
create or replace function public.list_minuta_provider_conversations_v161(
...
begin
  ...
  with page as (
    select conversation.*,booking.booking_code,booking.client_name,service.name service_name,
      coalesce((select message.body from public.conversation_messages_v161 message
      ...
      greatest(conversation.next_sequence-coalesce((select receipt.last_read_sequence
        from public.message_read_receipts_v161 receipt join public.message_participants_v161 participant
    ...
    from public.message_conversations_v161 conversation
    left join public.bookings booking on booking.id=conversation.primary_booking_id
    left join public.services service on service.id=booking.service_id
    where conversation.organization_id=p_organization
      and public.minuta_message_provider_can_access_v161(conversation.id)
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'can_send',page.state='open' and exists(select 1 from public.message_center_settings_v161 settings
...
create or replace function public.list_minuta_client_conversations_v161(
...
create or replace function public.capture_minuta_message_booking_event_v161()
...
begin
  for v_conversation in
    select conversation.id from public.message_conversations_v161 conversation
  ...
  loop
    if not exists(select 1 from public.conversation_system_events_v161 event
      where event.conversation_id=v_conversation.id and event.source_booking_event_id=new.id) then
      v_sequence:=public.minuta_message_next_sequence_v161(v_conversation.id);
      insert into public.conversation_system_events_v161(
...
create trigger booking_events_message_timeline_v161 after insert on public.booking_events
for each row execute function public.capture_minuta_message_booking_event_v161();

create or replace function public.propose_minuta_message_action_v161(
  p_conversation uuid,p_booking uuid,p_request_id uuid,p_action_type text,p_payload jsonb
) returns jsonb language plpgsql volatile security definer set search_path to '' as $$
declare v_actor uuid:=auth.uid();v_conversation public.message_conversations_v161%rowtype;v_booking public.bookings%rowtype;
...
begin
  if v_actor is null or p_request_id is null or not public.minuta_message_provider_can_access_v161(p_conversation)
  ...
  select * into v_conversation from public.message_conversations_v161
  ...
  v_response:=public.minuta_message_receipt_v161('provider:'||v_actor::text,'propose_action',p_request_id,v_hash);
  if v_response is not null then return v_response; end if;
  v_role:=public.minuta_message_provider_role_v161(v_conversation.organization_id);
  insert into public.message_participants_v161(conversation_id,participant_kind,user_id,participant_role)
  values(p_conversation,'organization_user',v_actor,case when v_role in('owner','admin') then v_role else 'specialist' end)
  on conflict do nothing;
  select id into v_sender from public.message_participants_v161 where conversation_id=p_conversation and user_id=v_actor and active;
  v_sequence:=public.minuta_message_next_sequence_v161(p_conversation);
  insert into public.conversation_messages_v161(conversation_id,sequence,sender_participant_id,message_kind,body,client_request_id,payload_sha256)
  values(p_conversation,v_sequence,v_sender,'action','',p_request_id,v_hash) returning id into v_message;
  insert into public.conversation_message_actions_v161(
    conversation_id,message_id,booking_id,action_type,public_payload,expected_booking_sha256,expires_at
  ) values(p_conversation,v_message,p_booking,p_action_type,p_payload,public.minuta_message_booking_sha256_v161(v_booking),v_expires)
  ...
  insert into public.message_idempotency_receipts_v161 values('provider:'||v_actor::text,'propose_action',p_request_id,v_hash,v_response,now());
  insert into public.message_audit_events_v161(organization_id,conversation_id,message_id,action_id,actor_kind,actor_ref,event_type,request_id,details)
...
create or replace function public.prepare_minuta_message_action_v161(
  p_session_token text,p_action uuid,p_request_id uuid
) returns jsonb language plpgsql volatile security definer set search_path to '' as $$
declare v_identity record;v_action public.conversation_message_actions_v161%rowtype;v_booking public.bookings%rowtype;
...
begin
  ...
  perform pg_advisory_xact_lock(hashtextextended('message-request:'||v_actor_key||':'||p_request_id::text,16001));
  v_response:=public.minuta_message_receipt_v161(v_actor_key,'prepare_action',p_request_id,v_request_hash);
  if v_response is not null then return v_response; end if;
  select * into v_action from public.conversation_message_actions_v161 where id=p_action;
  if v_action.id is null
     or not public.minuta_message_client_can_access_v161(p_session_token,v_action.conversation_id)
  ...
  if public.minuta_message_booking_sha256_v161(v_booking)<>v_action.expected_booking_sha256 then
    raise exception using errcode='40001',message='booking_changed';
  end if;
  v_token:=encode(extensions.gen_random_bytes(32),'hex');
  v_hash:=encode(extensions.digest(v_token,'sha256'),'hex');
  insert into public.message_action_confirmations_v161(action_id,client_session_id,request_id,token_hash,booking_sha256,expires_at)
  ...
  insert into public.message_audit_events_v161(organization_id,conversation_id,action_id,actor_kind,actor_ref,event_type,request_id,details)
...
create or replace function public.apply_minuta_message_action_v161(
  p_session_token text,p_action uuid,p_confirmation_token text,p_request_id uuid
) returns jsonb language plpgsql volatile security definer set search_path to '' as $$
declare v_identity record;v_action public.conversation_message_actions_v161%rowtype;v_confirmation public.message_action_confirmations_v161%rowtype;
...
begin
  ...
  perform pg_advisory_xact_lock(hashtextextended('message-request:'||v_actor_key||':'||p_request_id::text,16001));
  v_response:=public.minuta_message_receipt_v161(v_actor_key,'apply_action',p_request_id,v_request_hash);
  if v_response is not null then return v_response; end if;
  select * into v_action from public.conversation_message_actions_v161 where id=p_action for update;
  if not found or not public.minuta_message_client_can_access_v161(p_session_token,v_action.conversation_id)
  ...
  select * into v_confirmation from public.message_action_confirmations_v161
  ...
  update public.message_action_confirmations_v161 set consumed_at=now() where id=v_confirmation.id;
  if v_result='ok' then
    ...
    if not found then v_result:='booking_unavailable'; else
      ...
      if public.minuta_message_booking_sha256_v161(v_booking)<>v_confirmation.booking_sha256 then v_result:='booking_changed';
  ...
  update public.conversation_message_actions_v161 set state=case when v_result='ok' then 'applied' else 'failed' end,
  ...
  insert into public.message_idempotency_receipts_v161 values(v_actor_key,'apply_action',p_request_id,v_request_hash,v_response,now());
  insert into public.message_audit_events_v161(organization_id,conversation_id,action_id,actor_kind,actor_ref,event_type,request_id,details)
...
create or replace function public.open_minuta_provider_support_v161(
...
begin
  ...
  if not exists(select 1 from public.message_center_settings_v161 where organization_id=p_organization and support_enabled) then
    raise exception using errcode='P0001',message='message_support_disabled';
  end if;
  if not public.minuta_message_diagnostics_valid_v161(v_diag)
  ...
  perform pg_advisory_xact_lock(hashtextextended('message-request:'||v_actor_key||':'||p_request_id::text,16001));
  v_response:=public.minuta_message_receipt_v161(v_actor_key,'open_support',p_request_id,v_hash);
  if v_response is not null then return v_response; end if;
  insert into public.message_conversations_v161(conversation_kind,organization_id,subject)
  ...
  insert into public.message_support_requests_v161(
  ...
  perform public.minuta_send_message_core_v161(v_conversation,v_participant,'provider',v_actor_key,p_request_id,p_message);
  ...
  insert into public.message_idempotency_receipts_v161 values(v_actor_key,'open_support',p_request_id,v_hash,v_response,now());
  return v_response;
end
$$;

create or replace function public.open_minuta_client_support_v161(
...
begin
  ...
  if not exists(select 1 from public.message_center_settings_v161 where organization_id=v_booking.organization_id and support_enabled) then
    raise exception using errcode='P0001',message='message_support_disabled';
  end if;
  if not public.minuta_message_diagnostics_valid_v161(v_diag)
     or (not coalesce(p_diagnostics_consent,false) and v_diag<>'{}'::jsonb) then
  ...
  v_response:=public.minuta_message_receipt_v161(v_actor_key,'open_support',p_request_id,v_hash);
  if v_response is not null then return v_response; end if;
  insert into public.message_conversations_v161(conversation_kind,organization_id,primary_booking_id,client_account_id,subject)
  ...
  if v_identity.session_scope='booking' then
    insert into public.message_participants_v161(conversation_id,participant_kind,booking_id,participant_role)
    values(v_conversation,'booking_client',v_booking.id,'client') returning id into v_participant;
  else
    insert into public.message_participants_v161(conversation_id,participant_kind,client_account_id,participant_role)
  ...
  insert into public.message_support_requests_v161(
    ...
    case when p_diagnostics_consent and v_diag<>'{}'::jsonb then encode(extensions.digest(convert_to(v_diag::text,'UTF8'),'sha256'),'hex') end,
    case when p_diagnostics_consent then v_diag else '{}'::jsonb end) returning id into v_support;
  perform public.minuta_send_message_core_v161(v_conversation,v_participant,'client',v_actor_key,p_request_id,p_message);
  v_response:=jsonb_build_object('support_request_id',v_support,'conversation_id',v_conversation,
    'state','waiting_for_human','diagnostics_attached',p_diagnostics_consent and v_diag<>'{}'::jsonb,
    'created',true,'replayed',false);
  insert into public.message_idempotency_receipts_v161 values(v_actor_key,'open_support',p_request_id,v_hash,v_response,now());
...
create or replace function public.list_minuta_support_requests_v161(
...
begin
  if not exists(select 1 from public.message_support_agents_v161 where user_id=auth.uid() and active)
  ...
  into v_items from (select request.* from public.message_support_requests_v161 request
    join public.message_conversations_v161 conversation on conversation.id=request.conversation_id
    where (p_state is null or request.state=p_state)
      ...
      and (request.assigned_support_user_id is null or request.assigned_support_user_id=auth.uid()
        or exists(select 1 from public.message_support_agents_v161 lead where lead.user_id=auth.uid() and lead.support_role='lead' and lead.active))
...
create or replace function public.claim_minuta_support_request_v161(p_support_request uuid,p_request_id uuid)
returns jsonb language plpgsql volatile security definer set search_path to '' as $$
declare v_actor uuid:=auth.uid();v_request public.message_support_requests_v161%rowtype;v_response jsonb;v_hash text;
begin
  if v_actor is null or p_request_id is null or not exists(select 1 from public.message_support_agents_v161 where user_id=v_actor and active) then
  ...
  v_response:=public.minuta_message_receipt_v161('support:'||v_actor::text,'claim_support',p_request_id,v_hash);
  if v_response is not null then return v_response; end if;
  select * into v_request from public.message_support_requests_v161 where id=p_support_request for update;
  ...
  update public.message_support_requests_v161 set assigned_support_user_id=v_actor,state='assigned',updated_at=now()
  ...
  insert into public.message_idempotency_receipts_v161 values('support:'||v_actor::text,'claim_support',p_request_id,v_hash,v_response,now());
  return v_response;
end
$$;

create or replace function public.send_minuta_support_message_v161(p_conversation uuid,p_request_id uuid,p_body text)
returns jsonb language plpgsql volatile security definer set search_path to '' as $$
declare v_actor uuid:=auth.uid();v_sender uuid;
begin
  if v_actor is null or not public.minuta_message_support_can_access_v161(p_conversation) then
  ...
  return public.minuta_send_message_core_v161(p_conversation,v_sender,'support','support:'||v_actor::text,p_request_id,p_body);
end
$$;

create or replace function public.get_minuta_support_message_timeline_v161(
  p_conversation uuid,p_after_sequence bigint default null,p_before_sequence bigint default null,p_limit integer default 50
) returns jsonb language plpgsql stable security definer set search_path to '' as $$
begin
  if auth.uid() is null or not public.minuta_message_support_can_access_v161(p_conversation) then
    raise exception using errcode='42501',message='support_conversation_denied';
  end if;
  return public.minuta_message_timeline_core_v161(p_conversation,p_after_sequence,p_before_sequence,p_limit);
end
...
declare v_function regprocedure;v_expected text[]:=array[
  'apply_minuta_message_action_v161','capture_minuta_message_booking_event_v161','claim_minuta_support_request_v161',
  'get_minuta_client_message_capability_v161','get_minuta_client_message_timeline_v161','get_minuta_message_capability_v161',
  'get_minuta_provider_message_timeline_v161','get_minuta_support_message_timeline_v161','list_minuta_client_conversations_v161',
  'list_minuta_provider_conversations_v161','list_minuta_support_requests_v161','mark_minuta_client_message_read_v161',
  'mark_minuta_provider_message_read_v161','minuta_mark_message_read_core_v161','minuta_message_booking_sha256_v161',
  'minuta_message_client_can_access_v161','minuta_message_client_identity_v161','minuta_message_diagnostics_valid_v161',
  'minuta_message_ensure_booking_participants_v161','minuta_message_next_sequence_v161','minuta_message_provider_can_access_v161',
  'minuta_message_provider_role_v161','minuta_message_receipt_v161','minuta_message_support_can_access_v161',
  'minuta_message_timeline_core_v161','minuta_send_message_core_v161','open_minuta_client_conversation_v161',
  'open_minuta_client_support_v161','open_minuta_provider_conversation_v161','open_minuta_provider_support_v161',
  'prepare_minuta_message_action_v161','propose_minuta_message_action_v161','protect_minuta_message_immutable_v161',
  'send_minuta_client_message_v161','send_minuta_provider_message_v161','send_minuta_support_message_v161',
...
begin
  ...
  loop
    ...
    execute format('comment on function %s is %L',v_function,'minuta_message_center_v161');
  end loop;
end
$function_acl$;

grant execute on function public.get_minuta_message_capability_v161(uuid) to authenticated;
grant execute on function public.get_minuta_client_message_capability_v161(text,text) to anon,authenticated;
grant execute on function public.set_minuta_message_center_settings_v161(uuid,boolean,boolean) to authenticated;
grant execute on function public.open_minuta_provider_conversation_v161(uuid,uuid,uuid) to authenticated;
grant execute on function public.list_minuta_provider_conversations_v161(uuid,timestamptz,uuid,integer,text) to authenticated;
grant execute on function public.get_minuta_provider_message_timeline_v161(uuid,bigint,bigint,integer) to authenticated;
grant execute on function public.send_minuta_provider_message_v161(uuid,uuid,text) to authenticated;
grant execute on function public.mark_minuta_provider_message_read_v161(uuid,bigint) to authenticated;
grant execute on function public.propose_minuta_message_action_v161(uuid,uuid,uuid,text,jsonb) to authenticated;
grant execute on function public.open_minuta_provider_support_v161(uuid,uuid,text,jsonb,boolean) to authenticated;

grant execute on function public.open_minuta_client_conversation_v161(text,text,uuid) to anon,authenticated;
grant execute on function public.list_minuta_client_conversations_v161(text,timestamptz,uuid,integer,text) to anon,authenticated;
grant execute on function public.get_minuta_client_message_timeline_v161(text,uuid,bigint,bigint,integer) to anon,authenticated;
grant execute on function public.send_minuta_client_message_v161(text,uuid,uuid,text) to anon,authenticated;
grant execute on function public.mark_minuta_client_message_read_v161(text,uuid,bigint) to anon,authenticated;
grant execute on function public.prepare_minuta_message_action_v161(text,uuid,uuid) to anon,authenticated;
grant execute on function public.apply_minuta_message_action_v161(text,uuid,text,uuid) to anon,authenticated;
grant execute on function public.open_minuta_client_support_v161(text,text,uuid,text,jsonb,boolean) to anon,authenticated;

grant execute on function public.list_minuta_support_requests_v161(text,timestamptz,uuid,integer) to authenticated;
grant execute on function public.claim_minuta_support_request_v161(uuid,uuid) to authenticated;
grant execute on function public.send_minuta_support_message_v161(uuid,uuid,text) to authenticated;
grant execute on function public.get_minuta_support_message_timeline_v161(uuid,bigint,bigint,integer) to authenticated;
grant execute on function public.set_minuta_support_agent_v161(uuid,text,boolean) to service_role;
