...
with expected(name) as (values
 ('message_center_settings_v162'),('message_support_agents_v162'),('message_conversations_v162'),
 ('message_participants_v162'),('conversation_messages_v162'),('conversation_system_events_v162'),
 ('conversation_message_actions_v162'),('message_action_confirmations_v162'),('message_read_receipts_v162'),
 ('message_attachments_v162'),('message_support_requests_v162'),('message_idempotency_receipts_v162'),
 ('message_audit_events_v162')
), actual as (
 ...
 where namespace_row.nspname='public' and procedure_row.proname like '%\_v162' escape '\'
)
select jsonb_build_object('classification',case when count(relation)=0 and (select count(*) from functions)=0 then 'absent'
  when count(relation)=count(*) and bool_and(marker='minuta_message_center_v162'
    ...
    and not exists(select 1 from pg_class class_row join pg_namespace namespace_row on namespace_row.oid=class_row.relnamespace
      where namespace_row.nspname='public' and class_row.relkind='r' and class_row.relname like '%\_v162' escape '\'
  ...
  'allTablesMarked',coalesce(bool_and(marker is null or marker='minuta_message_center_v162'),false),
