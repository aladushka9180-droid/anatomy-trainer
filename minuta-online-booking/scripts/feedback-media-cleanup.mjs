...
export async function cleanupFeedbackMedia({baseUrl,expectedProjectRef,serviceKey,apply=false,fetchImpl=fetch}){
  ...
  if(!apply){
    const r=await request('/rest/v1/product_feedback_media_uploads?select=state&state=in.(reserved,cleanup)&limit=100');
  ...
  const claimed=await rpc('claim_minuta_feedback_cleanup_v116',{p_limit:50});
  if(!Array.isArray(claimed)||claimed.length>50||claimed.some(row=>!pathPattern.test(row?.path||'')||!tokenPattern.test(row?.token||''))
    ||new Set(claimed.map(row=>row.path)).size!==claimed.length)throw Error('invalid_cleanup_claim');
