...
const migration = readFileSync(new URL('../supabase-migration-v162.sql', import.meta.url), 'utf8').replace(/\r/g, '');
const tableStarts=[...migration.matchAll(/create table if not exists public\.([a-z0-9_]+_v162)\(/g)];
...
for(const match of migration.matchAll(/grant execute on function public\.([a-z0-9_]+_v162)\([^;]+?\)\s+to\s+([^;]+);/g))
  granted.set(match[1],new Set(match[2].split(',').map(role=>role.trim())));
const functions=[...migration.matchAll(/create or replace function public\.([a-z0-9_]+_v162)\s*\([\s\S]*?\)\s*returns[\s\S]*?\bas \$\$([\s\S]*?)\$\$;/g)]
  .map(match=>{const roles=granted.get(match[1])||new Set();return {name:match[1],
    sourceHash:createHash('sha256').update(match[2]).digest('hex'),marker:'minuta_message_center_v162',owner:'postgres',
...
process.stdout.write(JSON.stringify({version:'v162',tables,functions,sourceSha256,disabledByDefault:true,
