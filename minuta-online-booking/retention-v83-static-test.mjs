...
assert.match(controller, /setTimeout\(\(\) => saveSettings\(\), 500\)/);
assert.match(controller, /Сохранено автоматически/);
assert.match(controller, /includes\('\{ссылка\}'\)/);
assert.match(controller, /true, false\);/);
