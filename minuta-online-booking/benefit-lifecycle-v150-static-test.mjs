...
const rollback = readFileSync(new URL('./supabase-migration-v150-rollback.sql', import.meta.url), 'utf8');
...
assert.match(migration, /v_instrument\.status<>'active'[\s\S]*v_instrument\.expires_on<v_today[\s\S]*release_reserved_benefits_before_freeze/);
...
assert.match(migration, /procedure_row\.prosrc[\s\S]*procedure_row\.prosecdef[\s\S]*minuta_benefit_lifecycle_v150:sha256=/);
assert.match(migration, /pg_get_userbyid\(procedure_row\.proowner\)/);
assert.match(migration, /pg_get_userbyid\(relation_row\.relowner\)[\s\S]*relation_row\.relforcerowsecurity[\s\S]*pg_get_triggerdef/);
assert.match(migration, /pg_get_userbyid\(role_oid\)/);
assert.doesNotMatch(migration, /policy_row\.polroles::text/);
assert.match(rollback, /procedure_row\.prosrc[\s\S]*minuta_benefit_lifecycle_v150:sha256=/);
assert.ok(migration.indexOf('do $apply_version_guard$') < migration.indexOf('alter table public.benefit_ledger'));
assert.match(migration, /v_instrument\.status<>'frozen'[\s\S]*v_days:=public\.minuta_benefit_frozen_days_v150[\s\S]*expires_on=v_new_expiry/);
