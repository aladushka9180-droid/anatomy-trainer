...
assert.match(clientImport, /getExistingPhones = \(\) => \[\]/);
assert.match(clientImport, /Array\.isArray\(currentPhones\) \? currentPhones : \[\]/);
assert.match(clientImport, /preview\.fileName === 'phone-contacts' \? 'пропущено контактов'/);
assert.match(provider, /getExistingPhones: \(\) => buildClients\(\)\.map\(client => client\.phone\)/);
assert.doesNotMatch(provider, /getExistingPhones: \(\) => collectClients\(/);
assert.match(clientImport, /const IMPORT_BATCH_SIZE = 500/);
assert.match(clientImport, /importPreview\.rows\.slice\(offset, offset \+ IMPORT_BATCH_SIZE\)/);
assert.match(clientImport, /const importPreview = preview;[\s\S]*const organizationId = organization\.id;[\s\S]*const currentRevision = \+\+revision;/);
assert.match(clientImport, /import_minuta_booking_history', \{ p_organization:organizationId/);
assert.match(clientImport, /import_minuta_clients', \{ p_organization:organizationId/);
assert.ok((clientImport.match(/if \(!requestIsCurrent\(\)\) return;/g) || []).length >= 3, 'Смена организации должна останавливать многочастный импорт');
...
assert.doesNotMatch(html, /<script[^>]+vendor\/xlsx-0\.20\.3\.full\.min\.js/, 'Парсер Excel не должен замедлять старт кабинета');
assert.match(clientImport, /const XLSX_SOURCE = 'vendor\/xlsx-0\.20\.3\.full\.min\.js'/);
assert.match(clientImport, /const XLSX_INTEGRITY = 'sha384-EnyY0\/GSHQGSxSgMwaIPzSESbqoOLSexfnSMN2AP\+39Ckmn92stwABZynq1JyzdT'/);
