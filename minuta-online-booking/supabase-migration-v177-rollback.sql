-- Compatibility rollback: stop v177 writes and restore the v118 public shape.
-- Keep additive columns, expanded theme constraint and saved selections intact.
-- An older UI will display its safe fallback until the v177 UI is restored.
begin;

set local search_path = pg_catalog, public, extensions;

drop function if exists public.set_minuta_client_page_settings_v177(uuid,text,text,text,text);
drop function if exists public.get_minuta_client_page_settings_v177(uuid);
