...
assert.match(provider, /provider-themes-signature\.css\?v=643/, 'Кабинет не подключает Signature Collection v643');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=643/, 'Service Worker не кэширует Signature Collection v643');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=643/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=643/);
...
console.log('Signature themes v643: Snow Leopard canvas OK');
