...
assert.match(provider, /provider-themes-signature\.css\?v=588/, 'Кабинет не подключает Signature Collection v588');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=588/, 'Service Worker не кэширует Signature Collection v588');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=588/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=588/);
...
console.log('Signature themes v588: Snow Leopard canvas OK');
