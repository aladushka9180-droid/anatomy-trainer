...
assert.match(provider, /provider-themes-signature\.css\?v=640/, 'Кабинет не подключает Signature Collection v640');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=640/, 'Service Worker не кэширует Signature Collection v640');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=640/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=640/);
...
console.log('Signature themes v640: Snow Leopard canvas OK');
