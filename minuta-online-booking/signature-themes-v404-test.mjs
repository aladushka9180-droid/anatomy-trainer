...
assert.match(provider, /provider-themes-signature\.css\?v=648/, 'Кабинет не подключает Signature Collection v648');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=648/, 'Service Worker не кэширует Signature Collection v648');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=648/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=648/);
...
console.log('Signature themes v648: Snow Leopard canvas OK');
