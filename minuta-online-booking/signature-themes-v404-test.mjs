...
assert.match(provider, /provider-themes-signature\.css\?v=610/, 'Кабинет не подключает Signature Collection v610');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=610/, 'Service Worker не кэширует Signature Collection v610');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=610/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=610/);
...
console.log('Signature themes v610: Snow Leopard canvas OK');
