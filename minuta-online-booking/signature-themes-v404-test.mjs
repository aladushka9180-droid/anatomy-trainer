...
assert.match(provider, /provider-themes-signature\.css\?v=602/, 'Кабинет не подключает Signature Collection v602');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=602/, 'Service Worker не кэширует Signature Collection v602');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=602/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=602/);
...
console.log('Signature themes v602: Snow Leopard canvas OK');
