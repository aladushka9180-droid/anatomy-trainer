...
assert.match(provider, /provider-themes-signature\.css\?v=582/, 'Кабинет не подключает Signature Collection v582');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=582/, 'Service Worker не кэширует Signature Collection v582');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=582/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=582/);
...
console.log('Signature themes v582: Snow Leopard canvas OK');
