...
assert.match(provider, /provider-themes-signature\.css\?v=706/, 'Кабинет не подключает Signature Collection v706');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=706/, 'Service Worker не кэширует Signature Collection v706');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=706/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=706/);
...
console.log('Signature themes v706: Snow Leopard canvas OK');
