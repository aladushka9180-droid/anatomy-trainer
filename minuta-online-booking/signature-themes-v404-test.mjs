...
assert.match(provider, /provider-themes-signature\.css\?v=641/, 'Кабинет не подключает Signature Collection v641');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=641/, 'Service Worker не кэширует Signature Collection v641');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=641/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=641/);
...
console.log('Signature themes v641: Snow Leopard canvas OK');
