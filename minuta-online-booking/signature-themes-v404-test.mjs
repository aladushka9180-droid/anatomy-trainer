...
assert.match(provider, /provider-themes-signature\.css\?v=624/, 'Кабинет не подключает Signature Collection v624');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=624/, 'Service Worker не кэширует Signature Collection v624');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=624/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=624/);
...
console.log('Signature themes v624: Snow Leopard canvas OK');
