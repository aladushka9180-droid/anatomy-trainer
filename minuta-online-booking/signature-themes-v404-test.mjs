...
assert.match(provider, /provider-themes-signature\.css\?v=696/, 'Кабинет не подключает Signature Collection v696');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=696/, 'Service Worker не кэширует Signature Collection v696');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=696/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=696/);
...
console.log('Signature themes v696: Snow Leopard canvas OK');
