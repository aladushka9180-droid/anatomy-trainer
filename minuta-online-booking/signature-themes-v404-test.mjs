...
assert.match(provider, /provider-themes-signature\.css\?v=591/, 'Кабинет не подключает Signature Collection v591');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=591/, 'Service Worker не кэширует Signature Collection v591');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=591/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=591/);
...
console.log('Signature themes v591: Snow Leopard canvas OK');
