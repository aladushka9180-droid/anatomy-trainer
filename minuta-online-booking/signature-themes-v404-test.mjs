...
assert.match(provider, /provider-themes-signature\.css\?v=650/, 'Кабинет не подключает Signature Collection v650');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=650/, 'Service Worker не кэширует Signature Collection v650');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=650/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=650/);
...
console.log('Signature themes v650: Snow Leopard canvas OK');
