...
assert.match(provider, /provider-themes-signature\.css\?v=672/, 'Кабинет не подключает Signature Collection v672');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=672/, 'Service Worker не кэширует Signature Collection v672');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=672/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=672/);
...
console.log('Signature themes v672: Snow Leopard canvas OK');
