...
assert.match(provider, /provider-themes-signature\.css\?v=719/, 'Кабинет не подключает Signature Collection v719');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=719/, 'Service Worker не кэширует Signature Collection v719');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=719/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=719/);
...
console.log('Signature themes v719: Snow Leopard canvas OK');
