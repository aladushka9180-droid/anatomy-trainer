...
assert.match(provider, /provider-themes-signature\.css\?v=600/, 'Кабинет не подключает Signature Collection v600');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=600/, 'Service Worker не кэширует Signature Collection v600');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=600/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=600/);
...
console.log('Signature themes v600: Snow Leopard canvas OK');
