...
assert.match(provider, /provider-themes-signature\.css\?v=663/, 'Кабинет не подключает Signature Collection v663');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=663/, 'Service Worker не кэширует Signature Collection v663');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=663/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=663/);
...
console.log('Signature themes v663: Snow Leopard canvas OK');
