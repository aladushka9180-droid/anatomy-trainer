...
assert.match(provider, /provider-themes-signature\.css\?v=674/, 'Кабинет не подключает Signature Collection v674');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=674/, 'Service Worker не кэширует Signature Collection v674');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=674/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=674/);
...
console.log('Signature themes v674: Snow Leopard canvas OK');
