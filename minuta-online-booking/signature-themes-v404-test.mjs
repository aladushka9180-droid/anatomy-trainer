...
assert.match(provider, /provider-themes-signature\.css\?v=700/, 'Кабинет не подключает Signature Collection v700');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=700/, 'Service Worker не кэширует Signature Collection v700');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=700/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=700/);
...
console.log('Signature themes v700: Snow Leopard canvas OK');
