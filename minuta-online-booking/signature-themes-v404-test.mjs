...
assert.match(provider, /provider-themes-signature\.css\?v=580/, 'Кабинет не подключает Signature Collection v580');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=580/, 'Service Worker не кэширует Signature Collection v580');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=580/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=580/);
...
console.log('Signature themes v580: Snow Leopard canvas OK');
