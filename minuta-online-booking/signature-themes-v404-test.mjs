...
assert.match(provider, /provider-themes-signature\.css\?v=668/, 'Кабинет не подключает Signature Collection v668');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=668/, 'Service Worker не кэширует Signature Collection v668');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=668/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=668/);
...
console.log('Signature themes v668: Snow Leopard canvas OK');
