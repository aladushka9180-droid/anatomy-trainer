...
assert.match(provider, /provider-themes-signature\.css\?v=609/, 'Кабинет не подключает Signature Collection v609');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=609/, 'Service Worker не кэширует Signature Collection v609');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=609/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=609/);
...
console.log('Signature themes v609: Snow Leopard canvas OK');
