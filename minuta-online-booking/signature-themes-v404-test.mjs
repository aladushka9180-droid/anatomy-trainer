...
assert.match(provider, /provider-themes-signature\.css\?v=679/, 'Кабинет не подключает Signature Collection v679');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=679/, 'Service Worker не кэширует Signature Collection v679');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=679/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=679/);
...
console.log('Signature themes v679: Snow Leopard canvas OK');
