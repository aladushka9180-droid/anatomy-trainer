...
assert.match(provider, /provider-themes-signature\.css\?v=661/, 'Кабинет не подключает Signature Collection v661');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=661/, 'Service Worker не кэширует Signature Collection v661');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=661/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=661/);
...
console.log('Signature themes v661: Snow Leopard canvas OK');
