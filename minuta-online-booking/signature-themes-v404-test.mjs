...
assert.match(provider, /provider-themes-signature\.css\?v=682/, 'Кабинет не подключает Signature Collection v682');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=682/, 'Service Worker не кэширует Signature Collection v682');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=682/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=682/);
...
console.log('Signature themes v682: Snow Leopard canvas OK');
