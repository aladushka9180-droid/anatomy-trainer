...
assert.match(provider, /provider-themes-signature\.css\?v=604/, 'Кабинет не подключает Signature Collection v604');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=604/, 'Service Worker не кэширует Signature Collection v604');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=604/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=604/);
...
console.log('Signature themes v604: Snow Leopard canvas OK');
