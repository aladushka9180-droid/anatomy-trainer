...
assert.match(provider, /provider-themes-signature\.css\?v=688/, 'Кабинет не подключает Signature Collection v688');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=688/, 'Service Worker не кэширует Signature Collection v688');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=688/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=688/);
...
console.log('Signature themes v688: Snow Leopard canvas OK');
