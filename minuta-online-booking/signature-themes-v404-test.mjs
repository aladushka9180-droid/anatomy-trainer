...
assert.match(provider, /provider-themes-signature\.css\?v=690/, 'Кабинет не подключает Signature Collection v690');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=690/, 'Service Worker не кэширует Signature Collection v690');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=690/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=690/);
...
console.log('Signature themes v690: Snow Leopard canvas OK');
