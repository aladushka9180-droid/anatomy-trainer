...
assert.match(provider, /provider-themes-signature\.css\?v=586/, 'Кабинет не подключает Signature Collection v586');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=586/, 'Service Worker не кэширует Signature Collection v586');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=586/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=586/);
...
console.log('Signature themes v586: Snow Leopard canvas OK');
