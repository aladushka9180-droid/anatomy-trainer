...
assert.match(provider, /provider-themes-signature\.css\?v=594/, 'Кабинет не подключает Signature Collection v594');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=594/, 'Service Worker не кэширует Signature Collection v594');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=594/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=594/);
...
console.log('Signature themes v594: Snow Leopard canvas OK');
