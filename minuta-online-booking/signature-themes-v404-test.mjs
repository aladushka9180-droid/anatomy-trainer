...
assert.match(provider, /provider-themes-signature\.css\?v=659/, 'Кабинет не подключает Signature Collection v659');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=659/, 'Service Worker не кэширует Signature Collection v659');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=659/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=659/);
...
console.log('Signature themes v659: Snow Leopard canvas OK');
