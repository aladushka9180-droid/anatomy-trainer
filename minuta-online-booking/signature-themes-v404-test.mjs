...
assert.match(provider, /provider-themes-signature\.css\?v=578/, 'Кабинет не подключает Signature Collection v578');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=578/, 'Service Worker не кэширует Signature Collection v578');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=578/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=578/);
...
console.log('Signature themes v578: Snow Leopard canvas OK');
