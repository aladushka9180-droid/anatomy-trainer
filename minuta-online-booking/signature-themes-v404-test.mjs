...
assert.match(provider, /provider-themes-signature\.css\?v=673/, 'Кабинет не подключает Signature Collection v673');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=673/, 'Service Worker не кэширует Signature Collection v673');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=673/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=673/);
...
console.log('Signature themes v673: Snow Leopard canvas OK');
