...
assert.match(provider, /provider-themes-signature\.css\?v=585/, 'Кабинет не подключает Signature Collection v585');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=585/, 'Service Worker не кэширует Signature Collection v585');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=585/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=585/);
...
console.log('Signature themes v585: Snow Leopard canvas OK');
