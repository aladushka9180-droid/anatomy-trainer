...
assert.match(provider, /provider-themes-signature\.css\?v=709/, 'Кабинет не подключает Signature Collection v709');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=709/, 'Service Worker не кэширует Signature Collection v709');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=709/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=709/);
...
console.log('Signature themes v709: Snow Leopard canvas OK');
