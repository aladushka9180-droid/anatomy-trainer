...
assert.match(provider, /provider-themes-signature\.css\?v=599/, 'Кабинет не подключает Signature Collection v599');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=599/, 'Service Worker не кэширует Signature Collection v599');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=599/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=599/);
...
console.log('Signature themes v599: Snow Leopard canvas OK');
