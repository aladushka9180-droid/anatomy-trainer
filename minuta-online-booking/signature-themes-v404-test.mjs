...
assert.match(provider, /provider-themes-signature\.css\?v=566/, 'Кабинет не подключает Signature Collection v566');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=566/, 'Service Worker не кэширует Signature Collection v566');
...
assert.match(snowCanvas, /provider-snow-leopard-unified-landscape-v5\.png\?v=566/);
...
assert.match(portraitSnow, /provider-snow-leopard-unified-portrait-v6\.png\?v=566/, 'Portrait screens must use the native dense portrait canvas instead of zooming the landscape image');
...
for (const [asset, dimensions] of [['provider-snow-leopard-unified-landscape-v5.png', [1672, 941]], ['provider-snow-leopard-unified-portrait-v6.png', [941, 1672]]]) {
  ...
  assert.doesNotMatch(worker, new RegExp(`${asset.replace('.', '\\.') }\\?v=566`), `${asset} must be cached only after its theme is used`);
}
...
console.log('Signature themes v566: 13 unified themes OK');
