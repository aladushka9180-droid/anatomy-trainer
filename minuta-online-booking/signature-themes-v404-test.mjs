...
assert.match(provider, /provider-themes-signature\.css\?v=506/, 'Кабинет не подключает Signature Collection v506');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=506/, 'Service Worker не кэширует Signature Collection v506');
...
assert.match(snowCanvas, /provider-snow-leopard-unified-landscape-v5\.png\?v=506/);
...
assert.match(portraitSnow, /provider-snow-leopard-unified-portrait-v5\.png\?v=506/, 'Portrait screens must use a native portrait canvas instead of zooming the landscape image');
for (const [asset, dimensions] of [['provider-snow-leopard-unified-landscape-v5.png', [1672, 941]], ['provider-snow-leopard-unified-portrait-v5.png', [941, 1672]]]) {
  ...
  assert.match(worker, new RegExp(`${asset.replace('.', '\\.') }\\?v=506`), `${asset} is not available offline`);
}
...
console.log('Signature themes v506: 13 unified themes OK');
