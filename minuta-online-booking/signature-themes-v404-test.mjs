...
assert.match(provider, /provider-themes-signature\.css\?v=647/, 'Кабинет не подключает Signature Collection v647');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=647/, 'Service Worker не кэширует Signature Collection v647');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=647/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=647/);
...
console.log('Signature themes v647: Snow Leopard canvas OK');
