...
assert.match(provider, /provider-themes-signature\.css\?v=681/, 'Кабинет не подключает Signature Collection v681');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=681/, 'Service Worker не кэширует Signature Collection v681');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=681/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=681/);
...
console.log('Signature themes v681: Snow Leopard canvas OK');
