...
assert.match(provider, /provider-themes-signature\.css\?v=553/, 'Кабинет не подключает Signature Collection v553');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=553/, 'Service Worker не кэширует Signature Collection v553');
...
assert.match(snowCanvas, /provider-snow-leopard-unified-landscape-v5\.png\?v=553/);
...
assert.match(portraitSnow, /provider-snow-leopard-unified-portrait-v6\.png\?v=553/, 'Portrait screens must use the native dense portrait canvas instead of zooming the landscape image');
...
for (const [asset, dimensions] of [['provider-snow-leopard-unified-landscape-v5.png', [1672, 941]], ['provider-snow-leopard-unified-portrait-v6.png', [941, 1672]]]) {
  ...
  assert.match(worker, new RegExp(`${asset.replace('.', '\\.') }\\?v=553`), `${asset} is not available offline`);
}
...
console.log('Signature themes v553: 13 unified themes OK');
