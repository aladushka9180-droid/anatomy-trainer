...
assert.match(provider, /provider-themes-signature\.css\?v=535/, 'Кабинет не подключает Signature Collection v535');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=535/, 'Service Worker не кэширует Signature Collection v535');
...
assert.match(snowCanvas, /provider-snow-leopard-unified-landscape-v5\.png\?v=535/);
...
assert.match(portraitSnow, /provider-snow-leopard-unified-portrait-v6\.png\?v=535/, 'Portrait screens must use the native dense portrait canvas instead of zooming the landscape image');
...
for (const [asset, dimensions] of [['provider-snow-leopard-unified-landscape-v5.png', [1672, 941]], ['provider-snow-leopard-unified-portrait-v6.png', [941, 1672]]]) {
  ...
  assert.match(worker, new RegExp(`${asset.replace('.', '\\.') }\\?v=535`), `${asset} is not available offline`);
}
...
console.log('Signature themes v535: 13 unified themes OK');
