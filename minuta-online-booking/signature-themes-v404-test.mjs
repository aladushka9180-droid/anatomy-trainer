...
assert.match(provider, /provider-themes-signature\.css\?v=554/, 'Кабинет не подключает Signature Collection v554');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=554/, 'Service Worker не кэширует Signature Collection v554');
...
assert.match(snowCanvas, /provider-snow-leopard-unified-landscape-v5\.png\?v=554/);
...
assert.match(portraitSnow, /provider-snow-leopard-unified-portrait-v6\.png\?v=554/, 'Portrait screens must use the native dense portrait canvas instead of zooming the landscape image');
...
for (const [asset, dimensions] of [['provider-snow-leopard-unified-landscape-v5.png', [1672, 941]], ['provider-snow-leopard-unified-portrait-v6.png', [941, 1672]]]) {
  ...
  assert.match(worker, new RegExp(`${asset.replace('.', '\\.') }\\?v=554`), `${asset} is not available offline`);
}
...
console.log('Signature themes v554: 13 unified themes OK');
