...
assert.match(provider, /provider-themes-signature\.css\?v=626/, 'Кабинет не подключает Signature Collection v626');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=626/, 'Service Worker не кэширует Signature Collection v626');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=626/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=626/);
...
console.log('Signature themes v626: Snow Leopard canvas OK');
