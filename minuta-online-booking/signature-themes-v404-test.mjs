...
assert.match(provider, /provider-themes-signature\.css\?v=684/, 'Кабинет не подключает Signature Collection v684');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=684/, 'Service Worker не кэширует Signature Collection v684');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=684/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=684/);
...
console.log('Signature themes v684: Snow Leopard canvas OK');
