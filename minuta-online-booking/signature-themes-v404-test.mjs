...
assert.match(provider, /provider-themes-signature\.css\?v=583/, 'Кабинет не подключает Signature Collection v583');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=583/, 'Service Worker не кэширует Signature Collection v583');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=583/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=583/);
...
console.log('Signature themes v583: Snow Leopard canvas OK');
