...
assert.match(provider, /provider-themes-signature\.css\?v=666/, 'Кабинет не подключает Signature Collection v666');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=666/, 'Service Worker не кэширует Signature Collection v666');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=666/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=666/);
...
console.log('Signature themes v666: Snow Leopard canvas OK');
