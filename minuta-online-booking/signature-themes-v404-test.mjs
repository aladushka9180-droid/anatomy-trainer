...
assert.match(provider, /provider-themes-signature\.css\?v=619/, 'Кабинет не подключает Signature Collection v619');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=619/, 'Service Worker не кэширует Signature Collection v619');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=619/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=619/);
...
console.log('Signature themes v619: Snow Leopard canvas OK');
