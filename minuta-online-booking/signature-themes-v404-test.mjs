...
assert.match(provider, /provider-themes-signature\.css\?v=634/, 'Кабинет не подключает Signature Collection v634');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=634/, 'Service Worker не кэширует Signature Collection v634');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=634/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=634/);
...
console.log('Signature themes v634: Snow Leopard canvas OK');
