...
assert.match(provider, /provider-themes-signature\.css\?v=603/, 'Кабинет не подключает Signature Collection v603');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=603/, 'Service Worker не кэширует Signature Collection v603');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=603/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=603/);
...
console.log('Signature themes v603: Snow Leopard canvas OK');
