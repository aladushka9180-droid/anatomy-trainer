...
assert.match(provider, /provider-themes-signature\.css\?v=515/, 'Кабинет не подключает Signature Collection v515');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=515/, 'Service Worker не кэширует Signature Collection v515');
...
assert.match(snowCanvas, /provider-snow-leopard-unified-landscape-v5\.png\?v=515/);
...
assert.match(portraitSnow, /provider-snow-leopard-unified-portrait-v6\.png\?v=515/, 'Portrait screens must use the native dense portrait canvas instead of zooming the landscape image');
...
for (const [asset, dimensions] of [['provider-snow-leopard-unified-landscape-v5.png', [1672, 941]], ['provider-snow-leopard-unified-portrait-v6.png', [941, 1672]]]) {
  ...
  assert.match(worker, new RegExp(`${asset.replace('.', '\\.') }\\?v=515`), `${asset} is not available offline`);
}
...
console.log('Signature themes v515: 13 unified themes OK');
