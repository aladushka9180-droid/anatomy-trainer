...
assert.match(provider, /provider-themes-signature\.css\?v=605/, 'Кабинет не подключает Signature Collection v605');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=605/, 'Service Worker не кэширует Signature Collection v605');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=605/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=605/);
...
console.log('Signature themes v605: Snow Leopard canvas OK');
