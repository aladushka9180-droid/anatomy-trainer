...
assert.match(provider, /provider-themes-signature\.css\?v=705/, 'Кабинет не подключает Signature Collection v705');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=705/, 'Service Worker не кэширует Signature Collection v705');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=705/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=705/);
...
console.log('Signature themes v705: Snow Leopard canvas OK');
