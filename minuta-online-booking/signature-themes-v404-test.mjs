...
assert.match(provider, /provider-themes-signature\.css\?v=589/, 'Кабинет не подключает Signature Collection v589');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=589/, 'Service Worker не кэширует Signature Collection v589');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=589/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=589/);
...
console.log('Signature themes v589: Snow Leopard canvas OK');
