...
assert.match(provider, /provider-themes-signature\.css\?v=664/, 'Кабинет не подключает Signature Collection v664');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=664/, 'Service Worker не кэширует Signature Collection v664');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=664/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=664/);
...
console.log('Signature themes v664: Snow Leopard canvas OK');
