...
assert.match(provider, /provider-themes-signature\.css\?v=574/, 'Кабинет не подключает Signature Collection v574');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=574/, 'Service Worker не кэширует Signature Collection v574');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=574/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=574/);
...
console.log('Signature themes v574: Snow Leopard canvas OK');
