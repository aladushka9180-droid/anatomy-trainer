...
assert.match(provider, /provider-themes-signature\.css\?v=587/, 'Кабинет не подключает Signature Collection v587');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=587/, 'Service Worker не кэширует Signature Collection v587');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=587/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=587/);
...
console.log('Signature themes v587: Snow Leopard canvas OK');
