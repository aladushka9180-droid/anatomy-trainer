...
assert.match(provider, /provider-themes-signature\.css\?v=671/, 'Кабинет не подключает Signature Collection v671');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=671/, 'Service Worker не кэширует Signature Collection v671');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=671/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=671/);
...
console.log('Signature themes v671: Snow Leopard canvas OK');
