...
assert.match(provider, /provider-themes-signature\.css\?v=635/, 'Кабинет не подключает Signature Collection v635');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=635/, 'Service Worker не кэширует Signature Collection v635');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=635/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=635/);
...
console.log('Signature themes v635: Snow Leopard canvas OK');
