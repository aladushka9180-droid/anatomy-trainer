...
assert.match(provider, /provider-themes-signature\.css\?v=639/, 'Кабинет не подключает Signature Collection v639');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=639/, 'Service Worker не кэширует Signature Collection v639');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=639/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=639/);
...
console.log('Signature themes v639: Snow Leopard canvas OK');
