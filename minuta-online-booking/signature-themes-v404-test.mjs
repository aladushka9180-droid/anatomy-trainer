...
assert.match(provider, /provider-themes-signature\.css\?v=720/, 'Кабинет не подключает Signature Collection v720');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=720/, 'Service Worker не кэширует Signature Collection v720');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=720/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=720/);
...
console.log('Signature themes v720: Snow Leopard canvas OK');
