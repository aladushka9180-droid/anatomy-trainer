...
assert.match(provider, /provider-themes-signature\.css\?v=667/, 'Кабинет не подключает Signature Collection v667');
assert.match(worker, /\.\/provider-themes-signature\.css\?v=667/, 'Service Worker не кэширует Signature Collection v667');
...
assert.match(snowCanvas, /provider-snow-leopard-continuous-4k-v3\.webp\?v=667/);
...
assert.match(mobileSnow, /provider-snow-leopard-mobile-v1\.png\?v=667/);
...
console.log('Signature themes v667: Snow Leopard canvas OK');
