...
const generalWindows = voice.interpretCommand('Какие завтра окошки?', { today:'2026-09-02', services }, now);
assert.equal(generalWindows.kind, 'find_slots');
assert.equal(generalWindows.plan.general, true);
assert.equal(generalWindows.plan.date, '2026-09-03');
assert.equal(generalWindows.corrections, undefined, 'слово «окошки» не должно исправляться на единственное число');
