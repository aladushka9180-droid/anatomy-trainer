...
do $$
begin
  if to_regclass('public.organization_imported_clients') is null
     or to_regprocedure('public.import_minuta_clients(uuid,text,jsonb,uuid)') is null
     or to_regprocedure('public.get_minuta_staff_report_bookings(uuid,date,date,uuid)') is null then
    raise exception using errcode='P0001',message='v96_requires_v94_v95';
  end if;
end $$;
