...
try{
  ...
  await page.locator('#clientOverview').evaluate((host)=>{host.innerHTML=bookingClientOverviewMarkup(item);});
  const overview=page.locator('.booking-client-overview');
  assert.match(await overview.textContent(),/12 сент\. · 10:00/);
  assert.match(await overview.textContent(),/Визитов\s*4/);
  assert.match(await overview.textContent(),/Получено\s*2[\s ]?000 ₽/);
  assert.match(await overview.textContent(),/4 сент\. 2026 г\./);
