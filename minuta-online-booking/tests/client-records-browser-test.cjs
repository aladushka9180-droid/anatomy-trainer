...
(async()=>{
  ...
  try {
    ...
    assert.equal(await page.locator('#clientRecords > [data-cr-view="history"]:not([hidden])').count(),1,'history is the only initial records view');
    assert.equal(await page.locator('#clientRecords > [data-cr-view="history"] :is([data-cr-note],[data-cr-upload])').count(),0,'history contains visits only');
    await page.evaluate(()=>controller.setView('notes'));
    ...
    await page.evaluate(()=>{controller.setView('files');controller.setView('notes');});
    assert.match(await page.locator('[name="note"]').inputValue(),/спокойную музыку/,'view switches preserve an unsaved note');
    ...
    await page.locator('[data-cr-note] button').click();
    await page.waitForFunction(()=>document.querySelector('#clientRecords > [data-cr-view="notes"] .cr-timeline').textContent.includes('спокойную музыку'));
    ...
    await page.evaluate(()=>controller.setView('files'));
    ...
    assert.equal(await page.locator('#clientRecords > [data-cr-view="files"]:not([hidden])').count(),1,'files view is isolated');
    assert.equal(await page.locator('#clientRecords > [data-cr-view="files"] [data-cr-note]').count(),0,'files view does not repeat note controls');
    ...
    console.log('Client records: isolated views, note retry, XSS escaping, draft preservation, file upload, session isolation and 12 responsive/theme cases passed. Preview: '+preview);
