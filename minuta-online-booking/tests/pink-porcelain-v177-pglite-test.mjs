...
const [v118, v177, rollback] = await Promise.all([
  'supabase-migration-v118.sql', 'supabase-migration-v177.sql', 'supabase-migration-v177-rollback.sql'
...
try {
  ...
  const saved = await scalar(db, `select public.set_minuta_client_page_settings_v177('${organization}','pink-porcelain','beauty','petal-pink','silk')`);
  assert.deepEqual(saved.porcelain, { shade:'petal-pink', character:'silk' });
  const read = await scalar(db, `select public.get_minuta_client_page_settings_v177('${organization}')`);
  assert.deepEqual(read.porcelain, saved.porcelain);
  await expectError(db, `select public.set_minuta_client_page_settings_v177('${organization}','pink-porcelain','beauty','invalid','silk')`, 'client_page_porcelain_options_invalid');
  ...
  await expectError(db, `select public.set_minuta_client_page_settings_v177('${organization}','sage','care')`, 'permission denied');
  await db.exec('reset role;');
  await db.exec(`select set_config('request.jwt.claim.sub','${outsider}',false); set role authenticated;`);
  await expectError(db, `select public.get_minuta_client_page_settings_v177('${organization}')`, 'organization_read_denied');
  await expectError(db, `select public.set_minuta_client_page_settings_v177('${organization}','sage','care')`, 'organization_owner_required');
  await db.exec('reset role;');

  await db.exec(rollback);
  assert.equal(await scalar(db, "select to_regprocedure('public.set_minuta_client_page_settings_v177(uuid,text,text,text,text)') is null"), true);
  ...
  try {
    ...
    assert.equal(await scalar(restored, "select to_regprocedure('public.set_minuta_client_page_settings_v177(uuid,text,text,text,text)') is null"), true);
    await restored.exec(v177);
    assert.equal(await scalar(restored, "select to_regprocedure('public.set_minuta_client_page_settings_v177(uuid,text,text,text,text)') is not null"), true);
  } finally { await restored.close(); }
  console.log('Pink Porcelain v177 PGlite: tenant and role scope, validation, stale client safety, public shape, backup restore, rollback and reapply PASS');
