...
async function fixture(url) {
  ...
  const bookings = marks.filter(mark => !mark.includes('active')).map((mark, i) => `<article class="provider-booking status-confirmed color-sage ${mark}" data-card="booking-${i}"><button class="provider-booking-open" type="button"><span class="booking-time-column"><strong>11:00<small>до 12:30</small></strong><span>5 сентября</span></span><span class="booking-main"><span class="provider-booking-top"><h3>Массаж всего тела</h3><span class="booking-status">Подтверждена</span></span><span class="provider-booking-client-line"><span class="booking-client-name-row"><strong>Анна — тестовый клиент</strong>${badge(mark)}</span><span class="provider-booking-phone">+7 (900) 000-00-00</span></span><span class="provider-booking-signals"><span class="provider-booking-note-presence">Есть заметка</span></span></span><span class="provider-booking-chevron">›</span></button></article>`).join('');
  const timeline = marks.filter(mark => !mark.includes('active')).map((mark, i) => `<button type="button" class="timeline-booking status-confirmed color-sage ${mark}" data-card="timeline-${i}" data-booking-duration="90" style="position:relative;inset:auto;width:100%;height:190px;margin:8px 0"><span class="timeline-booking-time"><b>11:00</b><small>–12:30</small></span><span class="timeline-booking-copy"><strong>Массаж всего тела</strong><span class="timeline-booking-client-row"><small class="timeline-booking-client"><span class="timeline-mobile-time">11:00–12:30 · </span><span class="timeline-client-name">Анна — тестовый клиент</span><span class="timeline-client-phone">+7 (900) 000-00-00</span></small></span>${badge(mark)}<small class="timeline-booking-note-presence">Есть заметка</small></span><span class="timeline-booking-status">Подтверждена</span></button>`).join('');
  return `<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Тест карточек — без данных клиентов</title>${links}<style>.fixture{max-width:1100px;margin:0 auto;padding:16px}.fixture-columns{display:grid;grid-template-columns:1fr 1fr;gap:20px}.fixture .clients-list{max-height:none;overflow:visible}.fixture .client-list-item{width:100%}.fixture .provider-view{display:block}.fixture h2{font-size:22px}@media(max-width:760px){.fixture-columns{grid-template-columns:1fr}}</style></head><body class="provider-body" data-provider-theme="${theme}" data-provider-layout="${layout}" data-provider-text-scale="default"><main class="fixture" id="dashboard" data-active-view="bookings"><h2>Проверка оформления карточек</h2><p class="view-description" data-secondary>Только искусственные данные. Клиенты и записи не изменяются.</p><nav class="provider-section-nav"><button type="button" data-secondary>Клиенты</button><button type="button" class="active">Записи</button></nav><div class="fixture-columns"><section class="clients-directory"><div class="client-search"><input aria-label="Поиск клиента" placeholder="Имя или телефон"></div><div class="clients-list">${clients}</div></section><section class="provider-view" id="providerBookings"><div class="provider-bookings schedule-list">${bookings}</div><div class="provider-bookings timeline-view">${timeline}</div></section></div></main></body></html>`;
}
