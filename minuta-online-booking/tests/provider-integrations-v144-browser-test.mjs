...
try {
  for (const width of [390, 760, 1440]) {
    ...
    assert.match(text, /Черновик готов/);
    assert.match(text, /Обмен данными не начинается/);
    ...
    if (width === 390) {
      ...
      assert.match((await page.evaluate(() => window.integrationNotices.join(' '))), /тестовый черновик DIKIDI создан; обмен данными не начат/i);
    }
