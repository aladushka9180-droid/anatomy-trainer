...
assert.match(source, /historical\s*\? \{ allowPast:true, ignoreSchedule:true \}\s*:\s*\(!block && navigator\.onLine \? \{ ignoreAutomaticBuffer:true \} : undefined\)/);
assert.match(source, /voice-assistant-candidate[\s\S]{0,180}\{ ignoreAutomaticBuffer:true \}/);
...
async function slotFixture(serverSlots) {
  const fields = {
...
test('an unreleased automatic buffer explains why the preferred time is unavailable', async () => {
  ...
  assert.match(markup, /действует перерыв 60 мин/);
});
