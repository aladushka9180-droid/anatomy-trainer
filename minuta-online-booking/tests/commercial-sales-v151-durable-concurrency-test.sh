...
report_error() {
  local rc=$?
  printf 'v151 durable test failed at stage %s (exit %s); private SQL output withheld\n' "$stage" "$rc" >&2
  return "$rc"
}
stop_children() {
...
test "$(jq -S . <<<"$(counts)")" = "$(jq -S . <<<"$before_rollback")"
stage=seller-idempotency-conflict
...
stage=concurrent-result-equivalence
