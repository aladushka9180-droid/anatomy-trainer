...
assert.match(html, /value="cash">Наличными[\s\S]*value="manual">Подтверждена вручную/);
assert.match(html, /id="commercePaymentMethodHint"[\s\S]*Оплата подтверждена вручную\. Деньги в банке не проверяются/);
assert.match(controller, /methodHint\.hidden = .*commercePaymentMethod.* !== 'manual'/);
