...
assert.doesNotMatch(durable, /wait_event_type/);
assert.match(durable, /pg_advisory_xact_lock\(151,hashtext\('\$run_key:a-signal'\) & 2147483647\)/);
assert.match(durable, /trap finish EXIT/);
assert.match(durable, /select pg_sleep\(20\)/);
assert.match(durable, /stage=concurrent-lock-observation[\s\S]*for attempt in 1\.\.40 loop/);
assert.match(durable, /PGAPPNAME="\$app_b" psql[^\n]*>"\$tmp\/b\.log" 2>&1/);
assert.match(durable, /PGAPPNAME="\$app_a" psql[^\n]*>"\$tmp\/a\.log" 2>&1/);
assert.match(durable, /set local lock_timeout='30s'/);
assert.match(durable, /pg_advisory_xact_lock\(151,hashtext\('\$run_key:b-ready'\) & 2147483647\)/);
assert.match(durable, /minuta\.v151_observer_run_key/);
assert.match(durable, /primary_row\.pid=any\(pg_catalog\.pg_blocking_pids\(secondary_row\.pid\)\)/);
assert.match(durable, /waiting\.pid=secondary_row\.pid and not waiting\.granted/);
assert.match(durable, /serverLockWaitObserved:true/);
