...
const dayStates = changes => Array.from(calculate({ ...base, ...changes }).days, day => `${day.booking_date} ${day.status} ${day.max_duration_minutes}`);
const daily = changes => windows(changes).map(value => value.replace('2026-09-06 ', ''));
assert.deepEqual(daily(), ['10:00–20:00 600']);
assert.deepEqual(dayStates(), ['2026-09-06 available 600']);
...
assert.deepEqual(dayStates({ daysOff:[{ off_date:base.from, all_day:true }] }), ['2026-09-06 closed 0']);
...
assert.deepEqual(dayStates({ shifts:{ enabled:true, shifts:[], absences:[] } }), ['2026-09-06 not_scheduled 0']);
...
assert.deepEqual(dayStates({ bookings:[booking('09:00', 720)] }), ['2026-09-06 busy 0']);
assert.deepEqual(dayStates({ schedule:[{ weekday:1, enabled:true, start_time:'10:00', end_time:'20:00' }] }), ['2026-09-06 not_scheduled 0']);
assert.deepEqual(dayStates({ schedule:base.schedule.map(row => row.weekday === 7 ? { ...row, enabled:false } : row) }), ['2026-09-06 closed 0']);
