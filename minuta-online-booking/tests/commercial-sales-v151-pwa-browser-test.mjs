...
script.src='./site-update.js?v=752';
...
try {
  ...
  await page.waitForFunction(() => navigator.serviceWorker.controller?.scriptURL.endsWith('/sw.js?v=752'), null, { timeout:30000 });
  ...
  const state = await page.evaluate(async () => {
    ...
    const cache = await caches.open('massage-izhevsk-v752');
    const provider = await cache.match('./provider.html');
    const commerce = await cache.match('./commerce-management.js?v=752');
    const styles = await cache.match('./styles.css?v=752');
    return {
  ...
  assert.match(state.scriptUrl, /\/sw\.js\?v=752$/);
  assert.ok(state.cacheNames.includes('massage-izhevsk-v752'), 'v752 cache is installed');
  ...
  assert.match(state.providerSource, /styles\.css\?v=752/, 'fresh provider shell is stored in Cache Storage');
  assert.doesNotMatch(state.providerSource, /data-stale-provider-shell/, 'stale provider shell never reaches Cache Storage');
  assert.equal(state.commerce, true, 'v752 commerce controller is cached');
  assert.equal(state.styles, true, 'v752 UI styles are cached');
  ...
  const cachedSource = await page.evaluate(async () => (await fetch('./commerce-management.js?v=752')).text());
...
console.log('PrimeTime Pro commercial sales v151 PWA v752 and offline cache checks passed');
