...
script.src='./site-update.js?v=789';
...
try {
  ...
  await page.waitForFunction(() => navigator.serviceWorker.controller?.scriptURL.endsWith('/sw.js?v=789'), null, { timeout:30000 });
  ...
  const state = await page.evaluate(async () => {
    ...
    const cache = await caches.open('massage-izhevsk-v789');
    const provider = await cache.match('./provider.html');
    const commerce = await cache.match('./commerce-management.js?v=789');
    const styles = await cache.match('./styles.css?v=789');
    return {
  ...
  assert.match(state.scriptUrl, /\/sw\.js\?v=789$/);
  assert.ok(state.cacheNames.includes('massage-izhevsk-v789'), 'v789 cache is installed');
  ...
  assert.match(state.providerSource, /styles\.css\?v=789/, 'fresh provider shell is stored in Cache Storage');
  assert.doesNotMatch(state.providerSource, /data-stale-provider-shell/, 'stale provider shell never reaches Cache Storage');
  assert.equal(state.commerce, true, 'v789 commerce controller is cached');
  assert.equal(state.styles, true, 'v789 UI styles are cached');
  ...
  const cachedSource = await page.evaluate(async () => (await fetch('./commerce-management.js?v=789')).text());
...
console.log('PrimeTime Pro commercial sales v151 PWA v789 and offline cache checks passed');
