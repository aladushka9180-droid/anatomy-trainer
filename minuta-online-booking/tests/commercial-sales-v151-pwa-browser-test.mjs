...
script.src='./site-update.js?v=810';
...
try {
  ...
  await page.waitForFunction(() => navigator.serviceWorker.controller?.scriptURL.endsWith('/sw.js?v=810'), null, { timeout:30000 });
  ...
  const state = await page.evaluate(async () => {
    ...
    const cache = await caches.open('massage-izhevsk-v810');
    const provider = await cache.match('./provider.html');
    const commerce = await cache.match('./commerce-management.js?v=810');
    const styles = await cache.match('./styles.css?v=810');
    return {
  ...
  assert.match(state.scriptUrl, /\/sw\.js\?v=810$/);
  assert.ok(state.cacheNames.includes('massage-izhevsk-v810'), 'v810 cache is installed');
  ...
  assert.match(state.providerSource, /styles\.css\?v=810/, 'fresh provider shell is stored in Cache Storage');
  assert.doesNotMatch(state.providerSource, /data-stale-provider-shell/, 'stale provider shell never reaches Cache Storage');
  assert.equal(state.commerce, true, 'v810 commerce controller is cached');
  assert.equal(state.styles, true, 'v810 UI styles are cached');
  ...
  const cachedSource = await page.evaluate(async () => (await fetch('./commerce-management.js?v=810')).text());
...
console.log('PrimeTime Pro commercial sales v151 PWA v810 and offline cache checks passed');
