...
window.addEventListener('load',async()=>{
  ...
  script.src='./site-update.js?v=735';
...
try {
  ...
  const state = await page.evaluate(async () => {
    ...
    const cache = await caches.open('massage-izhevsk-v735');
    const provider = await cache.match('./provider.html');
    const commerce = await cache.match('./commerce-management.js?v=735');
    const styles = await cache.match('./styles.css?v=735');
    return {
  ...
  assert.match(state.scriptUrl, /\/sw\.js\?v=735$/);
  assert.ok(state.cacheNames.includes('massage-izhevsk-v735'), 'v735 cache is installed');
  ...
  assert.equal(state.commerce, true, 'v735 commerce controller is cached');
  assert.equal(state.styles, true, 'v735 UI styles are cached');
  ...
  const cachedSource = await page.evaluate(async () => (await fetch('./commerce-management.js?v=735')).text());
...
console.log('PrimeTime Pro commercial sales v151 PWA v735 and offline cache checks passed');
