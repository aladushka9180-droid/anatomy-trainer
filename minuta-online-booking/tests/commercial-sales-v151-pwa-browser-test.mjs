...
try {
  ...
  const state = await page.evaluate(async () => {
    ...
    const cache = await caches.open('massage-izhevsk-v738');
    const provider = await cache.match('./provider.html');
    const commerce = await cache.match('./commerce-management.js?v=738');
    const styles = await cache.match('./styles.css?v=738');
    return {
  ...
  assert.ok(state.cacheNames.includes('massage-izhevsk-v738'), 'v738 cache is installed');
  ...
  assert.equal(state.commerce, true, 'v738 commerce controller is cached');
  assert.equal(state.styles, true, 'v738 UI styles are cached');
  ...
  const cachedSource = await page.evaluate(async () => (await fetch('./commerce-management.js?v=738')).text());
...
console.log('PrimeTime Pro commercial sales v151 PWA v738 and offline cache checks passed');
