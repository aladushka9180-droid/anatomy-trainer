...
script.src='./site-update.js?v=743';
...
try {
  ...
  await page.waitForFunction(() => navigator.serviceWorker.controller?.scriptURL.endsWith('/sw.js?v=743'), null, { timeout:30000 });
  ...
  const state = await page.evaluate(async () => {
    ...
    const cache = await caches.open('massage-izhevsk-v743');
    const provider = await cache.match('./provider.html');
    const commerce = await cache.match('./commerce-management.js?v=743');
    const styles = await cache.match('./styles.css?v=743');
    return {
  ...
  assert.match(state.scriptUrl, /\/sw\.js\?v=743$/);
  assert.ok(state.cacheNames.includes('massage-izhevsk-v743'), 'v743 cache is installed');
  ...
  assert.match(state.providerSource, /styles\.css\?v=743/, 'fresh provider shell is stored in Cache Storage');
  assert.doesNotMatch(state.providerSource, /data-stale-provider-shell/, 'stale provider shell never reaches Cache Storage');
  assert.equal(state.commerce, true, 'v743 commerce controller is cached');
  assert.equal(state.styles, true, 'v743 UI styles are cached');
  ...
  const cachedSource = await page.evaluate(async () => (await fetch('./commerce-management.js?v=743')).text());
...
console.log('PrimeTime Pro commercial sales v151 PWA v743 and offline cache checks passed');
