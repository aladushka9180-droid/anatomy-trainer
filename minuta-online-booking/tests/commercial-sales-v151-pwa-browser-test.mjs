...
script.src='./site-update.js?v=790';
...
try {
  ...
  await page.waitForFunction(() => navigator.serviceWorker.controller?.scriptURL.endsWith('/sw.js?v=790'), null, { timeout:30000 });
  ...
  const state = await page.evaluate(async () => {
    ...
    const cache = await caches.open('massage-izhevsk-v790');
    const provider = await cache.match('./provider.html');
    const commerce = await cache.match('./commerce-management.js?v=790');
    const styles = await cache.match('./styles.css?v=790');
    return {
  ...
  assert.match(state.scriptUrl, /\/sw\.js\?v=790$/);
  assert.ok(state.cacheNames.includes('massage-izhevsk-v790'), 'v790 cache is installed');
  ...
  assert.match(state.providerSource, /styles\.css\?v=790/, 'fresh provider shell is stored in Cache Storage');
  assert.doesNotMatch(state.providerSource, /data-stale-provider-shell/, 'stale provider shell never reaches Cache Storage');
  assert.equal(state.commerce, true, 'v790 commerce controller is cached');
  assert.equal(state.styles, true, 'v790 UI styles are cached');
  ...
  const cachedSource = await page.evaluate(async () => (await fetch('./commerce-management.js?v=790')).text());
...
console.log('PrimeTime Pro commercial sales v151 PWA v790 and offline cache checks passed');
