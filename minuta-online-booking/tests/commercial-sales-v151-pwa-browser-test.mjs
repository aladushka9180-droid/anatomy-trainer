...
script.src='./site-update.js?v=796';
...
try {
  ...
  await page.waitForFunction(() => navigator.serviceWorker.controller?.scriptURL.endsWith('/sw.js?v=796'), null, { timeout:30000 });
  ...
  await page.evaluate(() => navigator.serviceWorker.controller?.postMessage({ type:'warm-provider-features' }));
  for (let attempt = 0; attempt < 100; attempt++) {
    const financeReady = await page.evaluate(async () => Boolean(await (await caches.open('massage-izhevsk-v796')).match('./finance-center.js?v=796')));
    if (financeReady) break;
    await new Promise(resolveWait => setTimeout(resolveWait, 20));
  }
  const state = await page.evaluate(async () => {
    const cacheNames = await caches.keys();
    const cache = await caches.open('massage-izhevsk-v796');
    const provider = await cache.match('./provider.html');
    const commerce = await cache.match('./commerce-management.js?v=796');
    const finance = await cache.match('./finance-center.js?v=796');
    const styles = await cache.match('./styles.css?v=796');
    return {
      ...
      commerce:Boolean(commerce), finance:Boolean(finance), styles:Boolean(styles)
    };
  });
  assert.match(state.scriptUrl, /\/sw\.js\?v=796$/);
  assert.ok(state.cacheNames.includes('massage-izhevsk-v796'), 'v796 cache is installed');
  ...
  assert.match(state.providerSource, /styles\.css\?v=796/, 'fresh provider shell is stored in Cache Storage');
  assert.doesNotMatch(state.providerSource, /data-stale-provider-shell/, 'stale provider shell never reaches Cache Storage');
  assert.equal(state.commerce, true, 'v796 commerce controller is cached');
  assert.equal(state.finance, true, 'v796 finance controller is cached');
  assert.equal(state.styles, true, 'v796 UI styles are cached');
  ...
  const cachedSource = await page.evaluate(async () => (await fetch('./commerce-management.js?v=796')).text());
...
console.log('PrimeTime Pro commercial sales v151 PWA v796 and offline cache checks passed');
