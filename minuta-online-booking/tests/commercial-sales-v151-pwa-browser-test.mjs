...
script.src='./site-update.js?v=801';
...
try {
  ...
  await page.waitForFunction(() => navigator.serviceWorker.controller?.scriptURL.endsWith('/sw.js?v=801'), null, { timeout:30000 });
  ...
  const state = await page.evaluate(async () => {
    ...
    const cache = await caches.open('massage-izhevsk-v801');
    const provider = await cache.match('./provider.html');
    const commerce = await cache.match('./commerce-management.js?v=801');
    const styles = await cache.match('./styles.css?v=801');
    return {
  ...
  assert.match(state.scriptUrl, /\/sw\.js\?v=801$/);
  assert.ok(state.cacheNames.includes('massage-izhevsk-v801'), 'v801 cache is installed');
  ...
  assert.match(state.providerSource, /styles\.css\?v=801/, 'fresh provider shell is stored in Cache Storage');
  assert.doesNotMatch(state.providerSource, /data-stale-provider-shell/, 'stale provider shell never reaches Cache Storage');
  assert.equal(state.commerce, true, 'v801 commerce controller is cached');
  assert.equal(state.styles, true, 'v801 UI styles are cached');
  ...
  const cachedSource = await page.evaluate(async () => (await fetch('./commerce-management.js?v=801')).text());
...
console.log('PrimeTime Pro commercial sales v151 PWA v801 and offline cache checks passed');
