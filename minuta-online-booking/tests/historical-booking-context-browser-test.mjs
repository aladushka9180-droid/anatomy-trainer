...
const functions=['openNewBookingSheet','createNewBooking','closeBookingSheet','setNewBookingMode','loadNewBookingSlots','renderNewBookingTimePicker','blockDurationChoices','activeProviderBlockContext','providerBlockLocationOptions','createOfflineBookingId','bookingMoveTimeIsPast',
...
for(const theme of ['snow-leopard','pearl-zebra','luxury']) for(const width of [390,1280]) cases.push([
  `UI booking and month grid ${theme} ${width}`,async page=>{
    ...
    assert.equal(await booking.evaluate(el=>el.matches(':focus-visible')),true);
    assert.equal(await booking.evaluate(el=>getComputedStyle(el).outlineWidth),'2px');
    await page.keyboard.press('Tab');
    assert.equal(await booking.evaluate(el=>getComputedStyle(el).boxShadow),'none','Highlight clears when focus moves');
