...
begin
  ...
  raise exception 'v178_synthetic:exclusion_did_not_block_overlap';
...
select pg_temp.v178_check((select booking.organization_id=
    ...
    where booking.request_id=current_setting('v178.provider_request')::uuid),
...
select pg_temp.v178_check((select booking.organization_id=
    ...
    from public.bookings booking
    where booking.id=(current_setting('v178.public_result')::jsonb->>'booking_id')::uuid),
  'team_scope_and_public_v92_attribution');
do $attribution$
begin
  update public.bookings set booking_source='provider_manual'
  where id=(current_setting('v178.public_result')::jsonb->>'booking_id')::uuid;
  raise exception 'v178_synthetic:attribution_was_mutable';
