...
const connect=async()=>{const c=new Client({connectionString:process.env.MINUTA_TEST_DATABASE_URL,
  application_name:'minuta-v161-message-concurrency',...(tls?{ssl:tls}:{})});await c.connect();clients.push(c);
...
try{
  ...
  await admin.query(`insert into public.message_center_settings_v161(organization_id,support_enabled,updated_by)
    values($1,true,$2); insert into public.message_conversations_v161(id,conversation_kind,organization_id,subject)
    values($3,'support',$1,'v161 concurrency'); insert into public.message_participants_v161
  ...
  const code=(await admin.query('select public.provider_book_appointment($1,$2,$3,$4,$5) code',
    [fixture.service,fixture.date,'10:00','V161 confirmation client','0000000000'])).rows[0].code;
  ...
  await admin.query(`insert into public.client_identity_sessions_v155(client_account_id,token_hash,session_scope,session_source,expires_at)
    ...
    insert into public.message_conversations_v161(id,conversation_kind,organization_id,primary_booking_id,client_account_id,next_sequence)
    values($3,'client',$4,$5,$1,1);
    insert into public.message_participants_v161(id,conversation_id,participant_kind,client_account_id,participant_role)
    values($6,$3,'client_account',$1,'client');
    insert into public.message_participants_v161(id,conversation_id,participant_kind,user_id,participant_role)
    values($7,$3,'organization_user',$8,'specialist');
    insert into public.conversation_messages_v161(id,conversation_id,sequence,sender_participant_id,message_kind,client_request_id,payload_sha256)
    values($9,$3,1,$7,'action',$10,$11);
    insert into public.conversation_message_actions_v161(id,conversation_id,message_id,booking_id,action_type,public_payload,expected_booking_sha256,expires_at)
    select $12,$3,$9,$5,'propose_time',jsonb_build_object('target_date',$13::date,'target_time','11:00'),
      public.minuta_message_booking_sha256_v161(booking),now()+interval '1 day' from public.bookings booking where booking.id=$5`,
  ...
  const otherCode=(await admin.query('select public.provider_book_appointment($1,$2,$3,$4,$5) code',
    [fixture.service,fixture.date,'12:00','V161 same-account isolation','0000000000'])).rows[0].code;
  ...
  assert.equal((await admin.query('select count(*)::int n from public.conversation_system_events_v161 where conversation_id=$1',[clientConversation])).rows[0].n,0,
  ...
  const sql='select public.minuta_send_message_core_v161($1,$2,$3,$4,$5,$6) result';
  ...
  assert.equal((await admin.query('select count(*)::int n from public.conversation_messages_v161 where conversation_id=$1',[conversation])).rows[0].n,1);
  assert.equal((await admin.query('select next_sequence from public.message_conversations_v161 where id=$1',[conversation])).rows[0].next_sequence,'1');
  ...
  const prepareSql='select public.prepare_minuta_message_action_v161($1,$2,$3) result';
  ...
  assert.equal((await admin.query('select count(*)::int n from public.message_action_confirmations_v161 where action_id=$1',[action])).rows[0].n,1);
  console.log('PASS: v161 concurrent send serializes lost-ACK replay and allocates unique sequences');
} finally {
  ...
  if(fixture?.actor){
    ...
    await admin.query(`delete from public.message_audit_events_v161 where organization_id=$1;
      delete from public.message_idempotency_receipts_v161 where actor_key=$2;
      delete from public.message_idempotency_receipts_v161 where actor_key like 'client-session:%' and response->>'action_id'=$3;
      delete from public.message_conversations_v161 where organization_id=$1;
      delete from public.message_center_settings_v161 where organization_id=$1`,[fixture.org,`support:${fixture.actor}`,fixture.action]);
    await admin.query('delete from public.client_identity_sessions_v155 where token_hash=$1',[sha256(fixture.sessionToken)]);
    await admin.query("select set_config('v161.cleanup_actor',$1,true),set_config('v161.cleanup_org',$2,true)",[fixture.actor,fixture.org]);
    await admin.query(`do $$ declare item record;v_actor uuid:=current_setting('v161.cleanup_actor')::uuid;
      v_org uuid:=current_setting('v161.cleanup_org')::uuid;begin
      for item in select c.table_name,c.column_name from information_schema.columns c join information_schema.tables t using(table_schema,table_name)
        where c.table_schema='public' and t.table_type='BASE TABLE' and c.column_name in('organization_id','performer_id')
          and c.table_name not like '%\_v161' escape '\' order by c.table_name loop
