...
create table pg_temp.v169_fixture(
  organization_id uuid,performer_id uuid,service_id uuid,
  first_due uuid,second_due uuid,too_early uuid,past_visit uuid,cancelled_visit uuid
) on commit drop;

do $fixture$
declare
  organization_id uuid:=gen_random_uuid(); performer_id uuid:=gen_random_uuid(); service_id uuid:=gen_random_uuid();
...
begin
  insert into pg_temp.v169_fixture values(
    organization_id,performer_id,service_id,first_due,second_due,too_early,past_visit,cancelled_visit
  );
  ...
  insert into public.bookings(
    id,booking_code,manage_token,organization_id,performer_id,service_id,
    client_name,client_phone,booking_date,booking_time,duration_minutes,
    original_price_rub,total_price_rub,status
  ) values
    (first_due,'V169-FIRST',gen_random_uuid(),organization_id,performer_id,service_id,'V169 client','79990000001',(local_now+interval '30 minutes')::date,(local_now+interval '30 minutes')::time,60,1000,1000,'confirmed'),
    (second_due,'V169-SECOND',gen_random_uuid(),organization_id,performer_id,service_id,'V169 client','79990000002',(local_now+interval '45 minutes')::date,(local_now+interval '45 minutes')::time,60,1000,1000,'confirmed'),
    (too_early,'V169-EARLY',gen_random_uuid(),organization_id,performer_id,service_id,'V169 client','79990000003',(local_now+interval '1471 minutes')::date,(local_now+interval '1471 minutes')::time,60,1000,1000,'confirmed'),
    (past_visit,'V169-PAST',gen_random_uuid(),organization_id,performer_id,service_id,'V169 client','79990000004',(local_now-interval '1 minute')::date,(local_now-interval '1 minute')::time,60,1000,1000,'confirmed'),
    (cancelled_visit,'V169-CANCEL',gen_random_uuid(),organization_id,performer_id,service_id,'V169 client','79990000005',(local_now+interval '20 minutes')::date,(local_now+interval '20 minutes')::time,60,1000,1000,'cancelled');
