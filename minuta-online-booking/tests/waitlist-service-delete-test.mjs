...
const run=new (Object.getPrototypeOf(async function(){}).constructor)('remove','confirm','db','notify','saveServiceScheduleName','refreshAfterWrite','servicePublicDetails',body);
for (const kind of ['waitlist','other','success','denied']) {
  ...
  await run({dataset:{deleteService:'service-a'}},()=>true,db,message=>messages.push(message),async(id,enabled)=>{calls.push(`short-name:${id}:${enabled}`);},async()=>calls.push('refresh'),new Map());
