...
function fixture({duration=15,serviceDuration=60,organization=true,online=true,responses=[],visibleBooking=true}={}){
 const fields={
  '#newBookingForm':{dataset:{}},'#newBookingSubmit':{disabled:false,textContent:'Занять время'},
  '#newBookingService':{value:'service-60'},'#newBookingDate':{value:'2099-01-04'},
 ...
 const sandbox={console,Date,JSON,Set,Number,String,navigator:{onLine:online},$:selector=>fields[selector],
  ...
  findCreatedBooking:()=>visibleBooking?{id:uuid}:null,ensureCreatedBookingVisible:async()=>null,
  ...
  db:{from:()=>{throw Error('direct booking UPDATE/DELETE forbidden');},rpc:async(name,args)=>{
   ...
   if(name==='get_provider_block_slots_v123')return {data:[{booking_time:'10:00:00'},{booking_time:'10:05:00'}],error:null};
   assert.equal(name,'create_provider_block_v123','no provider_book_appointment/book_appointment fallback');
...
test('actual duration-aware slots admit 15 minutes independently of a 60-minute technical service',async()=>{
 ...
 assert.deepEqual(f.calls,[{name:'get_provider_block_slots_v123',args:{p_organization:'organization-explicit',p_location:'branch-selected',
  p_service:'service-60',p_date:'2099-01-04',p_duration:15}}]);
...
test('actual create sends the complete exact-duration/tenant/request/title/note envelope',async()=>{
 ...
 assert.deepEqual(f.calls,[{name:'create_provider_block_v123',args:{p_organization:'organization-explicit',p_location:'branch-selected',
  p_service:'service-60',p_date:'2099-01-04',p_time:'10:00:00',p_duration:15,p_request_id:uuid,p_title:'Личный перерыв',p_note:'Важная заметка'}}]);
...
for(const [label,change] of [
 ...
 ['service',f=>{f.fields['#newBookingService'].value='another-service';}]
]){
...
for(const error of [{code:'PGRST202',message:'Could not find create_provider_block_v123 in the schema cache'},
 {code:'42501',message:'permission denied for function create_provider_block_v123'}]){
 test(`technical block failure ${error.code} never invokes legacy creation fallback`,async()=>{
  ...
  assert.deepEqual(f.calls.map(c=>c.name),['create_provider_block_v123']);assert.equal(f.writes.length,0);assert.ok(f.errors.length);
 });
}
