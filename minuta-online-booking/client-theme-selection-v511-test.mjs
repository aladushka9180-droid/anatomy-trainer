...
const expected = ['sage','nordic','warm','graphite','lavender','luxury','loft','eco','hitech','japandi','midnight','mono','desert','rose','botanical','burgundy','coastal','pearl','butter','celadon','snow-leopard','apricot-tiger','pearl-zebra','blue-hydrangea','peach-silk','moonlit-lilac','carbon-ember','petrol-steel','carbon-crimson','mocha-pastel','noir-safari'];
assert.deepEqual([...catalog.themeKeys], expected);
assert.equal(new Set(catalog.themeKeys).size, 31);
assert.equal(catalog.clientThemes.length, 24);
assert.deepEqual([...catalog.clientThemeKeys], expected.filter(key => !['blue-hydrangea','peach-silk','moonlit-lilac','carbon-ember','petrol-steel','carbon-crimson','mocha-pastel'].includes(key)));
...
assert.deepEqual(providerThemeKeys, expected, 'Каталог должен совпадать с 31 темой кабинета');
...
assert.match(providerHtml, /theme-catalog\.js\?v=686/);
assert.match(indexHtml, /id="clientThemeDialog"/);
assert.match(indexHtml, /id="clientHeroTitle"/);
assert.match(indexHtml, /client-themes\.css\?v=686/);
assert.match(indexHtml, /theme-catalog\.js\?v=686/);
...
assert.match(sw, /const CACHE = `\$\{CACHE_PREFIX\}v686`/);
assert.match(sw, /theme-catalog\.js\?v=686/);
...
console.log('client theme selection v686 tests passed');
