...
const expected = ['sage','nordic','warm','graphite','lavender','luxury','loft','eco','hitech','japandi','midnight','mono','desert','rose','botanical','burgundy','coastal','pearl','butter','celadon','snow-leopard','apricot-tiger','pearl-zebra','blue-hydrangea','peach-silk','moonlit-lilac','carbon-ember','petrol-steel','carbon-crimson','noir-safari'];
assert.deepEqual([...catalog.themeKeys], expected);
assert.equal(new Set(catalog.themeKeys).size, 30);
assert.equal(catalog.clientThemes.length, 24);
assert.deepEqual([...catalog.clientThemeKeys], expected.filter(key => !['blue-hydrangea','peach-silk','moonlit-lilac','carbon-ember','petrol-steel','carbon-crimson'].includes(key)));
...
assert.deepEqual(providerThemeKeys, expected, 'Каталог должен совпадать с 30 темами кабинета');
...
assert.match(providerHtml, /theme-catalog\.js\?v=685/);
assert.match(indexHtml, /id="clientThemeDialog"/);
assert.match(indexHtml, /id="clientHeroTitle"/);
assert.match(indexHtml, /client-themes\.css\?v=685/);
assert.match(indexHtml, /theme-catalog\.js\?v=685/);
...
assert.match(sw, /const CACHE = `\$\{CACHE_PREFIX\}v685`/);
assert.match(sw, /theme-catalog\.js\?v=685/);
...
console.log('client theme selection v685 tests passed');
