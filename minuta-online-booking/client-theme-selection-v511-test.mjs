...
const expected = ['sage','nordic','warm','graphite','lavender','luxury','loft','eco','hitech','japandi','midnight','mono','desert','rose','botanical','burgundy','coastal','pearl','butter','celadon','snow-leopard','apricot-tiger','pearl-zebra','noir-safari'];
assert.deepEqual([...catalog.themeKeys], expected);
assert.equal(new Set(catalog.themeKeys).size, 24);
...
assert.deepEqual(providerThemeKeys, expected, 'Каталог должен совпадать с 24 темами кабинета');
...
assert.match(providerHtml, /theme-catalog\.js\?v=633/);
assert.match(indexHtml, /id="clientThemeDialog"/);
assert.match(indexHtml, /id="clientHeroTitle"/);
assert.match(indexHtml, /client-themes\.css\?v=633/);
assert.match(indexHtml, /theme-catalog\.js\?v=633/);
...
assert.match(sw, /const CACHE = `\$\{CACHE_PREFIX\}v633`/);
assert.match(sw, /theme-catalog\.js\?v=633/);
...
console.log('client theme selection v633 tests passed');
