...
assert.equal(new Set(catalog.themeKeys).size, 32);
assert.equal(catalog.clientThemes.length, 24);
assert.deepEqual([...catalog.clientThemeKeys], expected.filter(key => !['blue-hydrangea','peach-silk','moonlit-lilac','carbon-ember','petrol-steel','carbon-crimson','mocha-pastel','oled-mono'].includes(key)));
...
assert.deepEqual(providerThemeKeys, expected, 'Каталог должен совпадать с 32 темами кабинета');
...
assert.match(providerHtml, /theme-catalog\.js\?v=710/);
assert.match(indexHtml, /id="clientThemeDialog"/);
assert.match(indexHtml, /id="clientHeroTitle"/);
assert.match(indexHtml, /client-themes\.css\?v=710/);
assert.match(indexHtml, /theme-catalog\.js\?v=710/);
...
assert.match(sw, /const CACHE = `\$\{CACHE_PREFIX\}v710`/);
assert.match(sw, /theme-catalog\.js\?v=710/);
...
console.log('client theme selection v710 tests passed');
