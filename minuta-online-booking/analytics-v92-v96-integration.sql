...
begin
  if to_regprocedure('public.get_minuta_team_analytics(uuid,date,date)') is null
    ...
    raise exception using errcode='P0001',message='v96_test_requires_v96';
  end if;
  if public.minuta_booking_value_v93(
    '{"service_id":null,"duration_minutes":60,"original_price_rub":1000,"total_price_rub":4321}'::jsonb,
    null
  ) <> 4321 then
    raise exception using errcode='P0001',message='v96_total_price_not_authoritative';
  end if;
  if not exists (
    select 1
    ...
    raise exception using errcode='P0001',message='v96_booking_event_delete_contract_failed';
  end if;
  if exists (
    select 1
    ...
    raise exception using errcode='P0001',message='v96_index_contract_failed';
  end if;
end $$;
...
insert into auth.users(
  ...
  'authenticated','authenticated','v96-foreign@example.invalid',now(),
  '{}'::jsonb,'{}'::jsonb,now(),now()
);
...
select set_config(
  ...
  (select organization.id::text
   from public.organizations organization
   where organization.legacy_performer_id=current_setting('minuta.v96_owner')::uuid
   order by organization.id limit 1),
  true
);
select set_config(
  ...
  (select location.id::text
   ...
   where location.organization_id=current_setting('minuta.v96_org')::uuid
     and location.active
...
select set_config(
  ...
  (select organization.public_slug
   from public.organizations organization
   where organization.id=current_setting('minuta.v96_org')::uuid),
  true
);
select set_config(
  'minuta.v96_slot',
  coalesce((
    ...
    from (
      ...
      join public.organization_memberships membership
        on membership.organization_id=current_setting('minuta.v96_org')::uuid
...
do $$
begin
  if current_setting('minuta.v96_owner',true) is null
     or current_setting('minuta.v96_org',true) is null
     or current_setting('minuta.v96_location',true) is null
     or current_setting('minuta.v96_slot',true)='{}' then
    raise exception using errcode='P0001',message='v96_test_fixture_missing';
  end if;
end $$;
...
values(
  ...
  'V95 second organization',
  'v96-second-organization',
  current_setting('minuta.v96_owner')::uuid
);
...
values(
  '00000000-0000-4000-8000-000000009501',
  current_setting('minuta.v96_owner')::uuid,
  'owner',false,true,current_setting('minuta.v96_owner')::uuid
);
select set_config('request.jwt.claim.sub',current_setting('minuta.v96_owner'),true);
set local role authenticated;
do $$
declare
  v_payload jsonb:=public.get_minuta_team_analytics(
    current_setting('minuta.v96_org')::uuid,current_date-31,current_date
  );
begin
  if (v_payload->>'organization_id')::uuid<>current_setting('minuta.v96_org')::uuid
     or (v_payload->>'can_view_team')::boolean is not true then
    raise exception using errcode='P0001',message='v96_explicit_organization_analytics_failed';
  end if;
end $$;
reset role;
...
begin
  if coalesce((v_payload->>'can_view_team')::boolean,true)
    ...
    raise exception using errcode='P0001',message='v96_cross_tenant_analytics_leak';
  end if;
end $$;
reset role;
...
select * from public.book_minuta_appointment(
  ...
  current_setting('minuta.v96_slug'),
  current_setting('minuta.v96_location')::uuid,
  (current_setting('minuta.v96_slot')::jsonb->>'service_id')::uuid,
  (current_setting('minuta.v96_slot')::jsonb->>'booking_date')::date,
  (current_setting('minuta.v96_slot')::jsonb->>'booking_time')::time,
  'V95 Delete Test','+79990009502'
);
reset role;
select set_config(
  'minuta.v96_booking',
...
begin
  if not exists (
    ...
    where event.booking_id=current_setting('minuta.v96_booking')::uuid
      and event.event_type='booking_created_online'
  ) then
    raise exception using errcode='P0001',message='v96_booking_event_not_created';
  end if;
end $$;
select set_config(
  'request.jwt.claim.sub',
  (current_setting('minuta.v96_slot')::jsonb->>'performer_id'),
  true
);
set local role authenticated;
do $$
begin
  if public.provider_delete_booking(current_setting('minuta.v96_booking')::uuid)<>'deleted' then
    raise exception using errcode='P0001',message='v96_provider_delete_failed';
  end if;
end $$;
reset role;
do $$
begin
  if exists(select 1 from public.bookings where id=current_setting('minuta.v96_booking')::uuid)
     or exists(select 1 from public.booking_events where booking_id=current_setting('minuta.v96_booking')::uuid) then
    raise exception using errcode='P0001',message='v96_booking_event_cascade_failed';
  end if;
end $$;
...
select * from public.book_minuta_appointment(
  ...
  (current_setting('minuta.v96_slot')::jsonb->>'booking_date')::date,
  (current_setting('minuta.v96_slot')::jsonb->>'booking_time')::time,
  'V95 Attribution Test','+79990009503'
);
reset role;
select set_config(
  'minuta.v96_booking',
...
where id=current_setting('minuta.v96_booking')::uuid;
...
begin
  begin
    ...
    where id=current_setting('minuta.v96_booking')::uuid;
    raise exception using errcode='P0001',message='v96_attribution_tamper_was_allowed';
...
begin
  if not exists (
    ...
    where booking.id=current_setting('minuta.v96_booking')::uuid
    ...
    raise exception using errcode='P0001',message='v96_attribution_set_null_failed';
  end if;
end $$;
