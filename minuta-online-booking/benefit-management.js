(function () {
  ...
  function createController(options) {
    ...
    function lifecycleIntent(instrumentId,action) {
      const key=`minuta_benefit_lifecycle_v150:${getCurrentUser()?.id}:${organization?.id}:${instrumentId}:${action}`;
      const raw=window.localStorage.getItem(key);
      if(raw){const value=JSON.parse(raw);if(value.organization_id!==organization?.id||value.instrument_id!==instrumentId||value.action!==action||!value.request_id)throw Error('invalid_lifecycle_intent');return {key,requestId:value.request_id};}
      const value={organization_id:organization.id,instrument_id:instrumentId,action,request_id:uuid()};
      const encoded=JSON.stringify(value);window.localStorage.setItem(key,encoded);
      if(window.localStorage.getItem(key)!==encoded)throw Error('lifecycle_intent_not_saved');
      return {key,requestId:value.request_id};
    }
