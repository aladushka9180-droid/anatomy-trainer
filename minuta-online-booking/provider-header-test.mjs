...
let availability=0;
const context=vm.createContext({$:()=>element,renderProviderVerification(){},recordConnectionEvent:(...args)=>events.push(args),applyWriteAvailability:()=>availability++,pendingBookingColors:new Set(),pendingBookingNotes:new Set(),pendingClientLabels:new Set(),pendingClientNotes:new Set()});
