...
drop index if exists public.bookings_organization_phone_v105_idx;
...
alter table public.booking_page_visits
  ...
  drop column if exists first_source_label,
...
begin
  ...
  loop
    ...
    if not exists (
      ...
      insert into public.booking_page_visits (organization_id, performer_id) values (v_organization, v_performer);
      v_inserted := true;
