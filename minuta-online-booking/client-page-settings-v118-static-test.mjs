...
assert.doesNotMatch(integration,/update public\.organization_memberships set role='admin'/i);
...
assert.doesNotMatch(app,/readClientOverride\(state\.organization\?\.id,\s*requestedOrganizationSlug\)/);
assert.doesNotMatch(app,/writeClientOverride\(state\.organization\?\.id,\s*event\.target\.value,\s*requestedOrganizationSlug\)/);
assert.match(app,/const effectiveSettings = catalog\.normalizeSettings\(state\.clientPage\)/);
...
assert.ok(provider.indexOf('if (!contextIsCurrent)') < provider.indexOf('await db.rpc(rpcName,args)'));
