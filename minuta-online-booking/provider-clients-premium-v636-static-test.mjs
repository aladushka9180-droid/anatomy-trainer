...
assert.match(html, /provider-schedule-minimal\.css\?v=806[\s\S]*provider-clients-premium\.css\?v=806/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=806[\s\S]*client-relationship\.js\?v=806[\s\S]*provider\.js\?v=806/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v806/);
...
console.log('Provider clients premium v806 static checks: PASS');
