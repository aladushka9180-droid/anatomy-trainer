...
assert.match(html, /provider-schedule-minimal\.css\?v=810[\s\S]*provider-clients-premium\.css\?v=810/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=810[\s\S]*client-relationship\.js\?v=810[\s\S]*provider\.js\?v=810/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v810/);
...
console.log('Provider clients premium v810 static checks: PASS');
