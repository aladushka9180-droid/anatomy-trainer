...
assert.match(html, /provider-schedule-minimal\.css\?v=790[\s\S]*provider-clients-premium\.css\?v=790/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=790[\s\S]*client-relationship\.js\?v=790[\s\S]*provider\.js\?v=790/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v790/);
...
console.log('Provider clients premium v790 static checks: PASS');
