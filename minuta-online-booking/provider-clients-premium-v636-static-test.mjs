...
assert.match(html, /provider-schedule-minimal\.css\?v=743[\s\S]*provider-clients-premium\.css\?v=743/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=743[\s\S]*client-relationship\.js\?v=743[\s\S]*provider\.js\?v=743/, 'Relationship rules must load before provider rendering');
for (const id of [
...
console.log('Provider clients premium v743 static checks: PASS');
