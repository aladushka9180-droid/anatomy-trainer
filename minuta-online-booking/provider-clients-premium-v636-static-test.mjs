...
assert.match(html, /provider-schedule-minimal\.css\?v=645[\s\S]*provider-clients-premium\.css\?v=645/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=645[\s\S]*client-relationship\.js\?v=645[\s\S]*provider\.js\?v=645/, 'Relationship rules must load before provider rendering');
for (const id of [
  'clientProfileOrbit', 'clientRelationshipTitle', 'clientRelationshipLevel', 'clientMilestoneCard',
  'clientMilestoneProgress', 'clientReliabilityCard', 'clientReliabilityText', 'clientVisits',
...
assert.match(css, /\.client-list-avatar-orbit\.has-photo[\s\S]*--client-photo-tone/);
assert.match(css, /@media \(max-width:760px\)/);
assert.match(css, /grid-template-columns:repeat\(2,minmax\(0,1fr\)\)/);

assert.match(worker, /CACHE_PREFIX}v645/);
...
console.log('Provider clients premium v645 static checks: PASS');
