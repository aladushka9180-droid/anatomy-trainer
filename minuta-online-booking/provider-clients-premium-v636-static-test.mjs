...
assert.match(html, /provider-schedule-minimal\.css\?v=798[\s\S]*provider-clients-premium\.css\?v=798/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=798[\s\S]*client-relationship\.js\?v=798[\s\S]*provider\.js\?v=798/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v798/);
...
console.log('Provider clients premium v798 static checks: PASS');
