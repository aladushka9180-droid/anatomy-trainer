...
assert.match(html, /provider-schedule-minimal\.css\?v=697[\s\S]*provider-clients-premium\.css\?v=697/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=697[\s\S]*client-relationship\.js\?v=697[\s\S]*provider\.js\?v=697/, 'Relationship rules must load before provider rendering');
for (const id of [
...
console.log('Provider clients premium v697 static checks: PASS');
