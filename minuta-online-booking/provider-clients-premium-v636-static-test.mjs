...
assert.match(html, /provider-schedule-minimal\.css\?v=801[\s\S]*provider-clients-premium\.css\?v=801/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=801[\s\S]*client-relationship\.js\?v=801[\s\S]*provider\.js\?v=801/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v801/);
...
console.log('Provider clients premium v801 static checks: PASS');
