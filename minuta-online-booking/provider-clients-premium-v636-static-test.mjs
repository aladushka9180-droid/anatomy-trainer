...
assert.match(html, /provider-schedule-minimal\.css\?v=789[\s\S]*provider-clients-premium\.css\?v=789/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=789[\s\S]*client-relationship\.js\?v=789[\s\S]*provider\.js\?v=789/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v789/);
...
console.log('Provider clients premium v789 static checks: PASS');
