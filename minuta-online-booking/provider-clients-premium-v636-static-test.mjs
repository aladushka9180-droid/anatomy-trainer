...
assert.match(html, /provider-schedule-minimal\.css\?v=804[\s\S]*provider-clients-premium\.css\?v=804/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=804[\s\S]*client-relationship\.js\?v=804[\s\S]*provider\.js\?v=804/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v804/);
...
console.log('Provider clients premium v804 static checks: PASS');
