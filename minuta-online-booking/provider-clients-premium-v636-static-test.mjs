...
assert.match(html, /provider-schedule-minimal\.css\?v=718[\s\S]*provider-clients-premium\.css\?v=718/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=718[\s\S]*client-relationship\.js\?v=718[\s\S]*provider\.js\?v=718/, 'Relationship rules must load before provider rendering');
for (const id of [
...
console.log('Provider clients premium v718 static checks: PASS');
