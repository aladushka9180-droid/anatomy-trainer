...
assert.match(html, /provider-schedule-minimal\.css\?v=794[\s\S]*provider-clients-premium\.css\?v=794/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=794[\s\S]*client-relationship\.js\?v=794[\s\S]*provider\.js\?v=794/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v794/);
...
console.log('Provider clients premium v794 static checks: PASS');
