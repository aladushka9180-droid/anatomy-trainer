...
assert.match(html, /provider-schedule-minimal\.css\?v=797[\s\S]*provider-clients-premium\.css\?v=797/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=797[\s\S]*client-relationship\.js\?v=797[\s\S]*provider\.js\?v=797/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v797/);
...
console.log('Provider clients premium v797 static checks: PASS');
