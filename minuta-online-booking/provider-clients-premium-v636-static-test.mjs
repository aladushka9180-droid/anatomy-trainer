...
assert.match(html, /provider-schedule-minimal\.css\?v=704[\s\S]*provider-clients-premium\.css\?v=704/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=704[\s\S]*client-relationship\.js\?v=704[\s\S]*provider\.js\?v=704/, 'Relationship rules must load before provider rendering');
for (const id of [
...
console.log('Provider clients premium v704 static checks: PASS');
