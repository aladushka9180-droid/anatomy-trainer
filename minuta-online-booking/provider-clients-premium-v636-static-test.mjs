...
assert.match(html, /provider-schedule-minimal\.css\?v=800[\s\S]*provider-clients-premium\.css\?v=800/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=800[\s\S]*client-relationship\.js\?v=800[\s\S]*provider\.js\?v=800/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v800/);
...
console.log('Provider clients premium v800 static checks: PASS');
