...
assert.match(html, /provider-schedule-minimal\.css\?v=793[\s\S]*provider-clients-premium\.css\?v=793/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=793[\s\S]*client-relationship\.js\?v=793[\s\S]*provider\.js\?v=793/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v793/);
...
console.log('Provider clients premium v793 static checks: PASS');
