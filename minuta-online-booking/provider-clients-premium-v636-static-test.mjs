...
assert.match(html, /provider-schedule-minimal\.css\?v=674[\s\S]*provider-clients-premium\.css\?v=674/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=674[\s\S]*client-relationship\.js\?v=674[\s\S]*provider\.js\?v=674/, 'Relationship rules must load before provider rendering');
for (const id of [
...
console.log('Provider clients premium v674 static checks: PASS');
