...
assert.match(html, /provider-schedule-minimal\.css\?v=696[\s\S]*provider-clients-premium\.css\?v=696/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=696[\s\S]*client-relationship\.js\?v=696[\s\S]*provider\.js\?v=696/, 'Relationship rules must load before provider rendering');
for (const id of [
...
console.log('Provider clients premium v696 static checks: PASS');
