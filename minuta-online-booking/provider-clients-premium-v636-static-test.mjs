...
assert.match(html, /provider-schedule-minimal\.css\?v=807[\s\S]*provider-clients-premium\.css\?v=807/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=807[\s\S]*client-relationship\.js\?v=807[\s\S]*provider\.js\?v=807/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v807/);
...
console.log('Provider clients premium v807 static checks: PASS');
