...
assert.match(html, /provider-schedule-minimal\.css\?v=652[\s\S]*provider-clients-premium\.css\?v=652/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=652[\s\S]*client-relationship\.js\?v=652[\s\S]*provider\.js\?v=652/, 'Relationship rules must load before provider rendering');
for (const id of [
...
console.log('Provider clients premium v652 static checks: PASS');
