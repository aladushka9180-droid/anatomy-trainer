...
assert.match(html, /provider-schedule-minimal\.css\?v=771[\s\S]*provider-clients-premium\.css\?v=771/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=771[\s\S]*client-relationship\.js\?v=771[\s\S]*provider\.js\?v=771/, 'Relationship rules must load before provider rendering');
for (const id of [
...
console.log('Provider clients premium v771 static checks: PASS');
