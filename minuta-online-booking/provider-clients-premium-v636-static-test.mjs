...
assert.match(html, /provider-schedule-minimal\.css\?v=809[\s\S]*provider-clients-premium\.css\?v=809/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=809[\s\S]*client-relationship\.js\?v=809[\s\S]*provider\.js\?v=809/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v809/);
...
console.log('Provider clients premium v809 static checks: PASS');
