...
assert.match(html, /provider-schedule-minimal\.css\?v=644[\s\S]*provider-clients-premium\.css\?v=644/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=644[\s\S]*client-relationship\.js\?v=644[\s\S]*provider\.js\?v=644/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v644/);
...
console.log('Provider clients premium v644 static checks: PASS');
