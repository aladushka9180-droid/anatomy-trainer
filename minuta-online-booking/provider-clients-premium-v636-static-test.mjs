...
assert.match(html, /provider-schedule-minimal\.css\?v=667[\s\S]*provider-clients-premium\.css\?v=667/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=667[\s\S]*client-relationship\.js\?v=667[\s\S]*provider\.js\?v=667/, 'Relationship rules must load before provider rendering');
for (const id of [
...
console.log('Provider clients premium v667 static checks: PASS');
