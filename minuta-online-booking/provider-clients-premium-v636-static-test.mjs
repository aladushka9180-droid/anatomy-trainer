...
assert.match(html, /provider-schedule-minimal\.css\?v=788[\s\S]*provider-clients-premium\.css\?v=788/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=788[\s\S]*client-relationship\.js\?v=788[\s\S]*provider\.js\?v=788/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v788/);
...
console.log('Provider clients premium v788 static checks: PASS');
