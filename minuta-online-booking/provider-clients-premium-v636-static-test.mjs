...
assert.match(html, /provider-schedule-minimal\.css\?v=653[\s\S]*provider-clients-premium\.css\?v=653/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=653[\s\S]*client-relationship\.js\?v=653[\s\S]*provider\.js\?v=653/, 'Relationship rules must load before provider rendering');
for (const id of [
...
console.log('Provider clients premium v653 static checks: PASS');
