...
assert.match(html, /provider-schedule-minimal\.css\?v=779[\s\S]*provider-clients-premium\.css\?v=779/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=779[\s\S]*client-relationship\.js\?v=779[\s\S]*provider\.js\?v=779/, 'Relationship rules must load before provider rendering');
for (const id of [
...
console.log('Provider clients premium v779 static checks: PASS');
