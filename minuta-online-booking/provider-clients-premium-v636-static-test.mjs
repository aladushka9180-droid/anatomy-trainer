...
assert.match(html, /provider-schedule-minimal\.css\?v=753[\s\S]*provider-clients-premium\.css\?v=753/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=753[\s\S]*client-relationship\.js\?v=753[\s\S]*provider\.js\?v=753/, 'Relationship rules must load before provider rendering');
for (const id of [
...
console.log('Provider clients premium v753 static checks: PASS');
