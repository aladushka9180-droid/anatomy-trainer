...
assert.match(html, /provider-schedule-minimal\.css\?v=643[\s\S]*provider-clients-premium\.css\?v=643/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=643[\s\S]*client-relationship\.js\?v=643[\s\S]*provider\.js\?v=643/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v643/);
...
console.log('Provider clients premium v643 static checks: PASS');
