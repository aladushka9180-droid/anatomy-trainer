...
assert.match(html, /provider-schedule-minimal\.css\?v=691[\s\S]*provider-clients-premium\.css\?v=691/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=691[\s\S]*client-relationship\.js\?v=691[\s\S]*provider\.js\?v=691/, 'Relationship rules must load before provider rendering');
for (const id of [
...
console.log('Provider clients premium v691 static checks: PASS');
