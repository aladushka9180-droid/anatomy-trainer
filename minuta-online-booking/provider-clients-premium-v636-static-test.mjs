...
assert.match(html, /provider-schedule-minimal\.css\?v=778[\s\S]*provider-clients-premium\.css\?v=778/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=778[\s\S]*client-relationship\.js\?v=778[\s\S]*provider\.js\?v=778/, 'Relationship rules must load before provider rendering');
for (const id of [
...
console.log('Provider clients premium v778 static checks: PASS');
