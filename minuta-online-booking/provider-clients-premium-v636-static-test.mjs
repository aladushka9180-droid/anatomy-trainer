...
assert.match(html, /provider-schedule-minimal\.css\?v=637[\s\S]*provider-clients-premium\.css\?v=637/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=637[\s\S]*client-relationship\.js\?v=637[\s\S]*provider\.js\?v=637/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v637/);
...
console.log('Provider clients premium v637 static checks: PASS');
