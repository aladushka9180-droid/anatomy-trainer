...
assert.match(html, /provider-schedule-minimal\.css\?v=646[\s\S]*provider-clients-premium\.css\?v=646/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=646[\s\S]*client-relationship\.js\?v=646[\s\S]*provider\.js\?v=646/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v646/);
...
console.log('Provider clients premium v646 static checks: PASS');
