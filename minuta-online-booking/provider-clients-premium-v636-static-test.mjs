...
assert.match(html, /provider-schedule-minimal\.css\?v=799[\s\S]*provider-clients-premium\.css\?v=799/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=799[\s\S]*client-relationship\.js\?v=799[\s\S]*provider\.js\?v=799/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v799/);
...
console.log('Provider clients premium v799 static checks: PASS');
