...
assert.match(html, /provider-schedule-minimal\.css\?v=808[\s\S]*provider-clients-premium\.css\?v=808/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=808[\s\S]*client-relationship\.js\?v=808[\s\S]*provider\.js\?v=808/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v808/);
...
console.log('Provider clients premium v808 static checks: PASS');
