...
assert.match(html, /provider-schedule-minimal\.css\?v=805[\s\S]*provider-clients-premium\.css\?v=805/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=805[\s\S]*client-relationship\.js\?v=805[\s\S]*provider\.js\?v=805/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v805/);
...
console.log('Provider clients premium v805 static checks: PASS');
