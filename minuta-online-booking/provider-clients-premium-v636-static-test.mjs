...
assert.match(html, /provider-schedule-minimal\.css\?v=756[\s\S]*provider-clients-premium\.css\?v=756/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=756[\s\S]*client-relationship\.js\?v=756[\s\S]*provider\.js\?v=756/, 'Relationship rules must load before provider rendering');
for (const id of [
...
console.log('Provider clients premium v756 static checks: PASS');
