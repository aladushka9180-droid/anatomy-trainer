...
assert.match(html, /provider-schedule-minimal\.css\?v=728[\s\S]*provider-clients-premium\.css\?v=728/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=728[\s\S]*client-relationship\.js\?v=728[\s\S]*provider\.js\?v=728/, 'Relationship rules must load before provider rendering');
for (const id of [
...
console.log('Provider clients premium v728 static checks: PASS');
