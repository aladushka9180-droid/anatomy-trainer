...
assert.match(html, /provider-schedule-minimal\.css\?v=795[\s\S]*provider-clients-premium\.css\?v=795/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=795[\s\S]*client-relationship\.js\?v=795[\s\S]*provider\.js\?v=795/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v795/);
...
console.log('Provider clients premium v795 static checks: PASS');
