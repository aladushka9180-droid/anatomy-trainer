...
assert.match(html, /provider-schedule-minimal\.css\?v=711[\s\S]*provider-clients-premium\.css\?v=711/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=711[\s\S]*client-relationship\.js\?v=711[\s\S]*provider\.js\?v=711/, 'Relationship rules must load before provider rendering');
for (const id of [
...
console.log('Provider clients premium v711 static checks: PASS');
