...
assert.match(html, /provider-schedule-minimal\.css\?v=636[\s\S]*provider-clients-premium\.css\?v=636/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=636[\s\S]*client-relationship\.js\?v=636[\s\S]*provider\.js\?v=636/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v636/);
...
console.log('Provider clients premium v636 static checks: PASS');
