...
assert.match(html, /provider-schedule-minimal\.css\?v=678[\s\S]*provider-clients-premium\.css\?v=678/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=678[\s\S]*client-relationship\.js\?v=678[\s\S]*provider\.js\?v=678/, 'Relationship rules must load before provider rendering');
for (const id of [
...
console.log('Provider clients premium v678 static checks: PASS');
