...
assert.match(html, /provider-schedule-minimal\.css\?v=647[\s\S]*provider-clients-premium\.css\?v=647/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=647[\s\S]*client-relationship\.js\?v=647[\s\S]*provider\.js\?v=647/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v647/);
...
console.log('Provider clients premium v647 static checks: PASS');
