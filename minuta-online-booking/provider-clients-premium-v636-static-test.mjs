...
assert.match(html, /provider-schedule-minimal\.css\?v=642[\s\S]*provider-clients-premium\.css\?v=642/, 'Client layer must load after all provider appearance layers');
assert.match(html, /client-results\.js\?v=642[\s\S]*client-relationship\.js\?v=642[\s\S]*provider\.js\?v=642/, 'Relationship rules must load before provider rendering');
for (const id of [
...
assert.match(worker, /CACHE_PREFIX}v642/);
...
console.log('Provider clients premium v642 static checks: PASS');
