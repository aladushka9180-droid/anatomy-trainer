...
assert.match(worker, /CACHE_PREFIX}v\d+/, 'Версия кэша приложения не определена');
...
assert.match(styles, /@media \(max-width:760px\)[\s\S]*\.report-view-tabs \{ position:sticky;[\s\S]*safe-area-inset-top/, 'mobile analytics tabs must remain reachable without covering the safe area');

console.log('analytics experience v400 checks passed');
