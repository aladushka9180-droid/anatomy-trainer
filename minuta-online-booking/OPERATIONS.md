...
## Безопасный порядок выпуска v64–v73
...
4. Проверить в тестовой базе все гарантии v61/v64, повторное применение v65–v73, tenant-backfill, отсутствие утечки между организациями, конкурентное бронирование ресурсов, ACL и полный обратный порядок отката.
...
6. Применить `supabase-migration-v64.sql`, затем v65–v73 строго в этом порядке. v68 атомарно откажется, если хотя одну старую запись нельзя однозначно привязать; последующие слои проверяют свои зависимости. Продолжать вручную после любой ошибки запрещено.
...
### Уведомления о посетителях v73

`supabase-migration-v73.sql` добавляет выключенную по умолчанию настройку
уведомлений, закрытый журнал посещений и публичную RPC регистрации события.
Событие не содержит имя, телефон, идентификатор устройства или посетителя.
Повторные открытия объединяются: одному специалисту создаётся не более одного
события за две минуты, история автоматически очищается через семь дней.

Для доставки при полностью закрытом приложении используется существующая функция
`telegram-booking-notify`: её webhook настраивается на вставку в
`booking_page_visits`. Функция принимает только запрос с секретным заголовком и
отправляет обезличенный текст в Telegram мастера. Событие посещения не связывается
с записью или клиентом.

Читать журнал может только сам специалист через RLS. Анонимному пользователю
разрешён только вызов ограниченной RPC для активной публичной организации; прямой
доступ к таблице запрещён. Для отката сначала отключить настройку, затем выполнить
`recovery/rollback-visitor-presence-v73.sql`.
