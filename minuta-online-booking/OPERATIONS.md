...
## Безопасный порядок выпуска v64–v67
...
4. Проверить в тестовой базе: чужая запись не удаляется, запись с отзывом возвращает `review_protected`, обычная запись удаляется вместе со связанными платёжными событиями, v65/v66/v67 повторно применяются без изменения данных, приглашение требует явного принятия подтверждённым email, последнего владельца нельзя удалить или понизить, а публичный каталог не смешивает организации и не открывается до серверного флага.
...
6. Применить `supabase-migration-v64.sql`, затем аддитивные `supabase-migration-v65.sql`, `v66` и `v67`. Миграция v67 атомарно включает `public_booking_enabled` ровно для настроенной основной организации; postcheck только подтверждает это состояние, отдельного ручного изменения нет. Раздел «Команда» совместим со старой схемой и покажет безопасное сообщение до активации БД; клиентский выбор специалиста включается только tenant-scoped ответом v67.
7. Запустить `Minuta production health`, затем `Minuta safe release gate` в режиме `observe-production` на 30–60 минут.

На Free Plan штатные копии Supabase недоступны. Исключение допускается только для миграции, которая не меняет таблицы и данные: сохранить точное определение изменяемой функции и ACL, записать количества и хеши идентификаторов, выполнить новую функцию внутри транзакции с `ROLLBACK`, затем применить её и повторно сверить показатели. Для любых изменений таблиц или данных эта замена резервной копии запрещена. Поэтому v65/v66/v67 можно подготовить и проверить транзакцией с `ROLLBACK`, но общий production-выпуск нельзя применять без подтверждённой внешней резервной копии.
