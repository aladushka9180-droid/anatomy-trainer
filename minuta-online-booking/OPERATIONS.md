...
## Безопасный порядок выпуска v64–v65
...
4. Проверить в тестовой базе: чужая запись не удаляется, запись с отзывом возвращает `review_protected`, обычная запись удаляется вместе со связанными платёжными событиями, а v65 повторно применяется без изменения идентификаторов организаций, филиалов и участников.
