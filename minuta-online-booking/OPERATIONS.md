...
## Безопасный порядок выпуска v64–v68
...
4. Проверить в тестовой базе все гарантии v61/v64, повторное применение v65–v68, tenant-backfill, отсутствие утечки между двумя организациями с одним специалистом, ACL и rollback v68→v67.
...
6. Применить `supabase-migration-v64.sql`, затем v65, v66, v67 и v68 строго в этом порядке. v68 атомарно откажется, если хотя одну старую запись нельзя однозначно привязать; продолжать вручную после этой ошибки запрещено.
7. Запустить `Minuta production health`, затем `Minuta safe release gate` в режиме `observe-production` на 30–60 минут.

На Free Plan штатные копии Supabase недоступны. Изменения можно проверять в production внутри одной транзакции с обязательным `ROLLBACK`, но это не заменяет backup. v68 меняет таблицу и данные, поэтому его production-выпуск без подтверждённой внешней резервной копии запрещён.
