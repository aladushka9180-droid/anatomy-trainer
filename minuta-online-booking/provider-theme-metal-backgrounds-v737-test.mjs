...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=799/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v799`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=799/);

console.log('Provider metal backgrounds v799: PASS');
