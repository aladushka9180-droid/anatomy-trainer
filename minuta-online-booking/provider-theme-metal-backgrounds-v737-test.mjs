...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=776/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v776`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=776/);

console.log('Provider metal backgrounds v776: PASS');
