...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=807/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v807`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=807/);

console.log('Provider metal backgrounds v807: PASS');
