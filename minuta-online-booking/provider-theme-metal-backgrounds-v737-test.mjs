...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=771/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v771`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=771/);

console.log('Provider metal backgrounds v771: PASS');
