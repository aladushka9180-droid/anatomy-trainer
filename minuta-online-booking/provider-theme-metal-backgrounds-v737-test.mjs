...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=751/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v751`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=751/);

console.log('Provider metal backgrounds v751: PASS');
