...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=788/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v788`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=788/);

console.log('Provider metal backgrounds v788: PASS');
