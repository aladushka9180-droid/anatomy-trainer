...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=766/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v766`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=766/);

console.log('Provider metal backgrounds v766: PASS');
