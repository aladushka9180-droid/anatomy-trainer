...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=801/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v801`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=801/);

console.log('Provider metal backgrounds v801: PASS');
