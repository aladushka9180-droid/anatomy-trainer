...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=768/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v768`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=768/);

console.log('Provider metal backgrounds v768: PASS');
