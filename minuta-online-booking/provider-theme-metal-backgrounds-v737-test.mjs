...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=750/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v750`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=750/);

console.log('Provider metal backgrounds v750: PASS');
