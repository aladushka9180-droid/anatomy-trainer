...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=742/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v742`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=742/);

console.log('Provider metal backgrounds v742: PASS');
