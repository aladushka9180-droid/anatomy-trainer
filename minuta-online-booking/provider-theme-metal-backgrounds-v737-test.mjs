...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=741/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v741`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=741/);

console.log('Provider metal backgrounds v741: PASS');
