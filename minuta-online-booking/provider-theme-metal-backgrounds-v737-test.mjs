...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=794/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v794`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=794/);

console.log('Provider metal backgrounds v794: PASS');
