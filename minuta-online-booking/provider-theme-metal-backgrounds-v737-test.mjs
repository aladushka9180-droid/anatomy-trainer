...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=782/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v782`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=782/);

console.log('Provider metal backgrounds v782: PASS');
