...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=773/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v773`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=773/);

console.log('Provider metal backgrounds v773: PASS');
