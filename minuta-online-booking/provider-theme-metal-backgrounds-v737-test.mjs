...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=767/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v767`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=767/);

console.log('Provider metal backgrounds v767: PASS');
