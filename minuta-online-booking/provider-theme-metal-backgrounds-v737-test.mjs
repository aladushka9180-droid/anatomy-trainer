...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=810/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v810`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=810/);

console.log('Provider metal backgrounds v810: PASS');
