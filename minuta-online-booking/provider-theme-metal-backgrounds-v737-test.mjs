...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=806/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v806`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=806/);

console.log('Provider metal backgrounds v806: PASS');
