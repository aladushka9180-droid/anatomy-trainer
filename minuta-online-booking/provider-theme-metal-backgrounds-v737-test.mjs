...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=792/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v792`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=792/);

console.log('Provider metal backgrounds v792: PASS');
