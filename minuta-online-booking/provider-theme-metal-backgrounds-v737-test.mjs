...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=761/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v761`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=761/);

console.log('Provider metal backgrounds v761: PASS');
