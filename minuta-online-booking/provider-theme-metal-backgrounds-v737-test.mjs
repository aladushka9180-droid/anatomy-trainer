...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=808/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v808`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=808/);

console.log('Provider metal backgrounds v808: PASS');
