...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=745/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v745`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=745/);

console.log('Provider metal backgrounds v745: PASS');
