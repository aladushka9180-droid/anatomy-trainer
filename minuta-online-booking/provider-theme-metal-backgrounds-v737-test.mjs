...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=759/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v759`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=759/);

console.log('Provider metal backgrounds v759: PASS');
