...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=758/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v758`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=758/);

console.log('Provider metal backgrounds v758: PASS');
