...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=795/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v795`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=795/);

console.log('Provider metal backgrounds v795: PASS');
