...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=780/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v780`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=780/);

console.log('Provider metal backgrounds v780: PASS');
