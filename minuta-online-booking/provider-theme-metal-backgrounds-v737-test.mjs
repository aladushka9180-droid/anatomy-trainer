...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=793/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v793`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=793/);

console.log('Provider metal backgrounds v793: PASS');
