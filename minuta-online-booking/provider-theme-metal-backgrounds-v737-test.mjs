...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=786/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v786`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=786/);

console.log('Provider metal backgrounds v786: PASS');
