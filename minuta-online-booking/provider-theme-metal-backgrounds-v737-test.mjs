...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=757/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v757`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=757/);

console.log('Provider metal backgrounds v757: PASS');
