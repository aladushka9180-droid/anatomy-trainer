...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=781/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v781`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=781/);

console.log('Provider metal backgrounds v781: PASS');
