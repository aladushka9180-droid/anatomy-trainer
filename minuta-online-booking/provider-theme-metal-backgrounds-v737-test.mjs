...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=762/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v762`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=762/);

console.log('Provider metal backgrounds v762: PASS');
