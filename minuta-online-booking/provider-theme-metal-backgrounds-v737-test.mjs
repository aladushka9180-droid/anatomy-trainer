...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=800/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v800`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=800/);

console.log('Provider metal backgrounds v800: PASS');
