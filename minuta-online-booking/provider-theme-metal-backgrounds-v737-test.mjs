...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=763/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v763`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=763/);

console.log('Provider metal backgrounds v763: PASS');
