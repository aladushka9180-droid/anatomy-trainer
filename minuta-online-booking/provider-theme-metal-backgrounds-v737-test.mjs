...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=772/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v772`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=772/);

console.log('Provider metal backgrounds v772: PASS');
