...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=785/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v785`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=785/);

console.log('Provider metal backgrounds v785: PASS');
