...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=760/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v760`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=760/);

console.log('Provider metal backgrounds v760: PASS');
