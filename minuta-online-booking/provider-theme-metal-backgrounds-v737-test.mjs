...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=748/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v748`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=748/);

console.log('Provider metal backgrounds v748: PASS');
