...
assert.match(html, /provider-theme-backgrounds-tema1\.css\?v=809/);
assert.doesNotMatch(html, /provider-theme-metal-backgrounds\.css/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v809`/);
assert.match(worker, /\.\/provider-theme-backgrounds-tema1\.css\?v=809/);

console.log('Provider metal backgrounds v809: PASS');
