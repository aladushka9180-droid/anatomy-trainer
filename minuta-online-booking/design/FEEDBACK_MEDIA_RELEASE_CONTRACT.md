...
- Для изолированного Storage-теста worker принимает явный `manifestPaths`: 1–50 различных канонических lower-case путей actor/request/attachment.ext (UUID той же формы, что принимает worker, расширение webp/mp4/webm/mov). `null`, пустой/разреженный массив, дубликаты, чужой формат и превышение 50 отклоняются до сетевого запроса. Список копируется до первого await. Без параметра остаётся прежний глобальный режим; CLI автоматически manifest не подставляет.
- Manifest-mode вызывает только `claim_minuta_feedback_cleanup_paths_v116(p_paths text[])`, отдельный service_role-only RPC. Валидация всей одномерной коллекции выполняется до мутации; `FOR UPDATE SKIP LOCKED` берёт только eligible строки `object_path=ANY(p_paths)`. Связанные и чужие eligible строки вне списка не меняются, включая их lease/token. Нельзя вызывать глобальный claim и затем фильтровать его ответ: это уже захватило бы чужие leases. Ошибка scoped RPC не вызывает fallback.
- Перед первым Storage DELETE worker проверяет, что все возвращённые пути принадлежат исходному manifest. Read-only preview тоже ограничен этим набором. Повтор scoped claim при активном lease возвращает пустой набор; после истечения lease выдаёт новый token, старый finish=false. Односессионные SQL/HTTP-тесты подтверждают эти границы, но не заменяют реальный multi-session/Storage тест. Обновление закреплённого SQL hash в concurrency CI выполняет владелец выпуска после QA.
