-- v137: repair production booking-funnel source analytics for services scoped through memberships.
begin;
...
begin
  if to_regclass('public.booking_funnel_events') is null
    ...
    raise exception 'v137_booking_funnel_prerequisites_missing';
  end if;
end
$guard$;
...
begin
  ...
  if p_service is not null and not exists(
    ...
    join public.organization_memberships membership
      on membership.organization_id=v_org and membership.user_id=service.performer_id
...
begin
  select pg_get_functiondef('public.track_public_booking_funnel_event(text,uuid,text,uuid,uuid,text,text,text,text,text,text,text)'::regprocedure) into v_definition;
  if position('join public.organization_memberships membership' in lower(v_definition))=0
     or position('service.organization_id' in lower(v_definition))>0
     ...
     or not has_function_privilege('authenticated','public.track_public_booking_funnel_event(text,uuid,text,uuid,uuid,text,text,text,text,text,text,text)','EXECUTE')
     or not has_function_privilege('service_role','public.track_public_booking_funnel_event(text,uuid,text,uuid,uuid,text,text,text,text,text,text,text)','EXECUTE')
     or has_table_privilege('anon','public.booking_funnel_events','SELECT,INSERT,UPDATE,DELETE') then
    raise exception 'v137_booking_funnel_postcondition_failed';
  end if;
end
$postcondition$;
