...
begin
  ...
  insert into public.financial_transactions(organization_id,request_id,request_fingerprint,operation_type,source_type,source_id,source_fingerprint,occurred_at,explanation,created_by)
    values(p_organization,md5(p_request_id::text||':finance')::uuid,v_fingerprint,'commercial_refund','commercial_sale_refund',v_refund,v_fingerprint,now(),jsonb_build_object('schema','minuta-commerce-v1','sale_id',p_sale,'refund_id',v_refund,'quantity',p_quantity,'amount_minor',p_amount_minor,'reason',btrim(p_reason)),v_actor) returning id into v_transaction;
