...
for (const html of [index, booking]) {
  ...
  assert.match(html, /telegram-auth\.js\?v=357/, 'Telegram authorization controller is not loaded');
}
...
assert.match(sw, /'\.\/telegram-auth\.js\?v=357'/, 'Telegram authorization controller is not cached');
assert.match(setup, /\/setdomain/, 'Required BotFather domain setup is not documented');

console.log('Telegram web authorization v357 checks passed');
