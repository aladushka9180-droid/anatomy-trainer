...
async function loadBookings() {
  ...
  let { data, error } = await db.rpc('get_client_bookings_v2', { p_session_token: sessionToken });
  if (error) ({ data, error } = await db.rpc('get_client_bookings', { p_session_token: sessionToken }));
...
openAccount();
if ('serviceWorker' in navigator) window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=126'));
