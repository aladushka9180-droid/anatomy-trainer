...
assert.match(provider, /Добавить визит · учесть/);
...
assert.match(provider, /newBookingHistoricalMode && newBookingPreferredTime && bookingMoveTimeIsPast/);
assert.match(styles, /\.new-booking-historical-payment \{/);
assert.match(styles, /grid-template-columns:minmax\(0,1fr\) minmax\(0,1fr\)/);
assert.match(html, /provider\.js\?v=695/);
assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}v695`/);
assert.match(worker, /\.\/provider\.js\?v=695/);
