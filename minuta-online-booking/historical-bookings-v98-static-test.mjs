...
assert.match(provider, /Boolean\(preset\.historical\)/);
assert.doesNotMatch(provider, /Boolean\ Magnitude\(preset\.historical \|\| draft\?\.historical\)/);
assert.match(provider, /p_duration_minutes:durationMinutes/);
const historicalCall = provider.match(/db\.rpc\('create_minuta_historical_booking',[\s\S]{0,1200}?\n\s*\}\);/i)?.[0] || '';
assert.ok(historicalCall, 'historical booking RPC call must exist');
assert.doesNotMatch(historicalCall, /p_total_price_rub/);
...
assert.match(provider, /data-create-booking-at[\s\S]{0,300}role="group"[\s\S]{0,300}tabindex="0"/);
assert.doesNotMatch(provider, /data-create-booking-at[^>]+role="slider"/);
