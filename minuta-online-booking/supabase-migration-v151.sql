...
begin
  if to_regclass('public.commercial_sales') is null
     ...
     or to_regprocedure('public.get_minuta_commerce_workspace_v147(uuid)') is null then
    raise exception using errcode='55000',message='v151_requires_commercial_sales_v147';
  end if;
end
$guard$;
...
begin
  ...
  perform pg_advisory_xact_lock(hashtextextended(p_organization::text||':commerce:'||p_request_id::text,151));
