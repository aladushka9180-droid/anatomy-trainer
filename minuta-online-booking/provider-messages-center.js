(function initializeProviderMessagesCenter(global) {
  ...
  const PROVIDER_RPC_MAP = Object.freeze({
    capability:'get_minuta_message_capability_v161',
    openConversation:'open_minuta_provider_conversation_v161',
    listConversations:'list_minuta_provider_conversations_v161',
    timeline:'get_minuta_provider_message_timeline_v161',
    send:'send_minuta_provider_message_v161',
    markRead:'mark_minuta_provider_message_read_v161',
    proposeAction:'propose_minuta_message_action_v161',
    openSupport:'open_minuta_provider_support_v161'
  });
  ...
  function createMessageCenter(options) {
    ...
    async function initialize() {
      ...
      if (!capability.enabled) {
        ...
        setNotice('Сообщения ещё не подключены', capability.reason || 'Схема v161 или доступ организации не подтверждены. Остальные разделы работают.');
  ...
  async function mount(options = {}) {
    ...
    try {
      ...
      root.classList.add('message-center');
      root.innerHTML = '<div class="message-center-error"><div><strong>Сообщения пока недоступны</strong><p>Контекст организации или схема v161 не подтверждены. Остальные разделы продолжают работать.</p></div></div>';
      return null;
    }
  }
