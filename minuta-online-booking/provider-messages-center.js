(function initializeProviderMessagesCenter(global) {
  ...
  function createMessageCenter(options) {
    ...
    root.innerHTML = `<section class="message-center-shell" aria-labelledby="messageCenterTitle">
      <header class="message-center-heading"><div class="message-center-heading-copy"><small class="message-center-eyebrow">${actorKind === 'provider' ? 'Связь с клиентами' : 'Личный раздел'}</small><h1 id="messageCenterTitle" tabindex="-1">Сообщения</h1><p>Переписка по записям и отдельная помощь PrimeTime.</p>${actorKind === 'provider' ? '<nav class="message-center-context-links" aria-label="Связанные разделы"><button type="button" data-provider-view="notifications">Доставка уведомлений</button><button type="button" data-open-notification-templates>Шаблоны сообщений</button></nav>' : ''}</div><span class="message-center-state" data-message-connection role="status" aria-live="polite">Проверяем доступ…</span></header>
