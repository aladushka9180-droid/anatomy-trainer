...
В production нужен один Supabase Cron, вызывающий `notification-dispatcher` раз в минуту.
...
каналов пустой вызов обязан вернуть `claimed: 0` без ошибок. Задача
`telegram-client-reminders-hourly` после v114 не используется.
...
4. Применить v114 и развернуть обновлённый `telegram-client-notify`: `/event` теперь только
   читает статус outbox, а прямой ежечасовой маршрут напоминаний выведен из эксплуатации.
